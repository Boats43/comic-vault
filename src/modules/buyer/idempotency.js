// src/modules/buyer/idempotency.js — PRIVATE. Mirrors
// src/modules/assets/idempotency.js exactly (same class-wide GK-163
// law, same shared data1_dev.idempotency_key table — operation names
// used here, 'appendBuyerDecision'/'appendBuyerAcquisition', are
// distinct from every operation name assets/auth already use, so no
// collision is possible in that shared table). Duplicated rather than
// imported from src/modules/assets/ because that file is itself PRIVATE
// to the assets module (tests/assets-module-boundary.test.js) — this is
// a second, independent implementation of the same small, generic
