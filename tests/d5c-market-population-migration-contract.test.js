...
// Applicability judgment), MP-NP4 (changed rule version produces a new
// historical population), MP-NP5 (a later population does not
// overwrite an earlier one), MP-NP6 (duplicate execution dedups),
// MP-NP7 (legitimate reevaluation not blocked), MP-NP12 (no population
// operation mutates MarketObservation/ValuationQuestion/historical
// Applicability).
//
...
try {
  ...
  await assertSucceeds(
    () => client.query(
      `INSERT INTO market_population_member (id, market_population_id, observation_id, applicability_id, member_status)
       VALUES ($1,$2,$3,$4,'SELECTED')`,
      [member1Id, pop1Id, observationId1, applicabilityId1]
    ),
  ...
  await assertSucceeds(
    () => client.query(
      `INSERT INTO market_population_member (id, market_population_id, observation_id, applicability_id, member_status)
       VALUES ($1,$2,$3,$4,'SELECTED')`,
      [member2Id, pop1Id, observationId2, applicabilityId2]
    ),
  ...
  await assertRejected(
    () => client.query(
      `INSERT INTO market_population_member (id, market_population_id, observation_id, applicability_id, member_status)
       VALUES ($1,$2,$3,$4,'SELECTED')`,
      [crypto.randomUUID(), pop1Id, crypto.randomUUID(), applicabilityId1]
    ),
  ...
  await assertRejected(
    () => client.query(
      `INSERT INTO market_population_member (id, market_population_id, observation_id, applicability_id, member_status)
       VALUES ($1,$2,$3,$4,'SELECTED')`,
      [crypto.randomUUID(), mismatchTestPopId, observationId1, applicabilityId2] // applicabilityId2 judged observationId2, not observationId1
    ),
  ...
  await assertRejected(
    () => client.query(
      `INSERT INTO market_population_member (id, market_population_id, observation_id, applicability_id, member_status, exclusion_reason)
       VALUES ($1,$2,$3,$4,'SELECTED','should not be allowed')`,
      [crypto.randomUUID(), pop1Id, observationId1, applicabilityId1]
    ),
  ...
  await assertSucceeds(
    () => client.query(
      `INSERT INTO market_population_member (id, market_population_id, observation_id, applicability_id, member_status, exclusion_reason)
       VALUES ($1,$2,$3,$4,'EXCLUDED','dedup: near-identical listing, lower price kept')`,
      [crypto.randomUUID(), shapeCheckPopId, observationId1, applicabilityId1]
    ),
  ...
  assertTrue(allPops.rows.length === 5, `MP-NP5: all 5 population header rows (pop1 v1, mismatch-test, shape-check, pop2 v2, pop3 v1-reeval) coexist -- none overwritten (actual: ${allPops.rows.length})`);
  ...
  for (let i = 0; i < 20; i++) {
    ...
    await client.query(
      `INSERT INTO market_population_member (id, market_population_id, observation_id, applicability_id, member_status)
       VALUES ($1,$2,$3,$4,'SELECTED')`,
      [crypto.randomUUID(), memberWalPopId, pair.obsId, pair.apId]
    );
  }
    ...
    return lsnDiffBytes(async () => {
      for (const { obsId, apId } of batchPairs) {
        await client.query(
          `INSERT INTO market_population_member (id, market_population_id, observation_id, applicability_id, member_status)
           VALUES ($1,$2,$3,$4,'SELECTED')`,
          [crypto.randomUUID(), memberWalPopId, obsId, apId]
        );
      }
    });
  };
  ...
  const appUkGone = await client.query(`SELECT conname FROM pg_constraint WHERE conrelid = $1::regclass AND conname = 'applicability_id_observation_uk'`, [`${SCHEMA}.applicability`]);
  assertTrue(appUkGone.rows.length === 0, 'applicability_id_observation_uk constraint (added by 0017) removed by rollback');
