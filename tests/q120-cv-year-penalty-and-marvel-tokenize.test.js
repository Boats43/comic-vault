...
// behavior (Finding 1: lookupComicVine's scoreWithDetails year-gap
// penalty). ComicVine is disabled (Dispatch 42, api/enrich.js
// COMICVINE_ENABLED), not deleted, and this suite goes live again
// automatically once it's re-enabled. No new production export is added
// solely for this check — reads the existing source of truth directly.
// Assertions below are unmodified; this only short-circuits before any
// of them run when the adapter is off. Fails OPEN (runs normally) if the
// flag can't be parsed at all, so a renamed/refactored constant surfaces
// as a normal test failure rather than a silent permanent skip.
{
