...
**Evidence (current state), as originally published 2026-09-01, preserved for chronology:** `decision_event` exists and is live (`db/data0/0004_data1_foundation.sql:192`). The outcome side does not: `db/data0/0006_outcome_ledger.sql` states in its own header, "**DESIGN DRAFT, NOT APPLIED**... Not applied to `data1_dev` or any database as part of this dispatch" (lines 1-6), and defines `outcome_event`/`asset_outcome_current` only as an unapplied draft. No `OperatorAction`, `ListingProjection`, `Transaction`, or `PredictionError` table exists in any applied migration. Consistent with "PARTIAL / design-only-in-practice."

**Evidence correction (added U2 Ratification, 2026-09-21) — same "correct stale evidence, preserve chronology, do not silently rewrite" discipline already used for the Media Durability (GK-167) entry below.** The paragraph above is now stale: `operator_action_event` (migration `0021_operator_action_event.sql`, live) and `outcome_event`/`outcome_economics_component` (migrations `0023_outcome1_marketplace_execution_ledger.sql`/`0025_outcome1_economics_components.sql`, live) were built and applied to `data1_dev` afterward, via GK-203/205/209/216's Outcome #1 train — a **different** migration than the one the original evidence paragraph cites; `0006_outcome_ledger.sql` itself was never applied and was effectively superseded, not completed. `ListingProjection` and a durable `PredictionError` table still do not exist in any applied migration — `PredictionError` exists only as a pure, non-persisted scoring function (`src/lib/predictionErrorScoring.js`). **This is an evidence correction only, per explicit U2 ruling: the law's own wording above and its "Audited status: PARTIAL / design-only-in-practice" line are deliberately left unchanged by this correction** — a status change requires its own explicit audit ruling, per the Status governance section below, and is not made here. Full detail: `docs/U1-UNIVERSAL-CENSUS.md`, C12 and the U1 Review Packet addendum.
...
## Universal Kernel Boundary (ratified, U2, 2026-09-21)

**Type: ARCHITECTURE/DOCUMENTATION RATIFICATION ONLY.** Ratified against `main` commit `836f264` (GK-238, SHIPPED), following the U1 Universalization Census (`docs/U1-UNIVERSAL-CENSUS.md`) and its follow-up review packet. No runtime code, schema migration, or pricing change accompanies this section — every claim below is a ratification of an already-existing repo fact, cited to its source, not a new build. This section extends the Foundation Laws' own scope (principally Laws 1, 2, and 4/8/13 of the Compatibility Matrix) to the specific question U1/U2 were asked to resolve: how universal is the kernel today, and what does that mean for the next build phase (Generic Asset Mode, U4).

**Canonical framing (ratified verbatim):**

> GrailKey's kernel is architecturally category-neutral by construction. Non-comic Production behavior remains unproven until U4 creates and retrieves the first deliberate non-comic physical asset.

This sentence governs how every other ratification in this section is read — "universal" below always means "universal by construction, not yet Production-proven with a non-comic asset." No claim in this section should be read as asserting the reverse.

**Ratified kernel scope.** The following are ratified as universal (category-neutral) durable kernel concepts, each already implemented as a live, category-agnostic mechanism as of `836f264`: `gkAssetId` (`gk_asset.id`); principal/ownership (`gk_principal`, `ownership_event`, `current_owner`); `asset_class` (`gk_asset.asset_class`, see the controlled-vocabulary ruling below); physical evidence/media (`media`, content-addressed, category-agnostic `media_type`/`capture_view` vocabulary); provenance (`entity_mint_basis`/`mint_event`); acquisition/cost basis (`acquisition_event`); valuation history (`valuation_event`); Buyer Mode decisions (`buyer_decision_event`/`buyer_acquisition_event`, economics columns only — see the identity-boundary ruling below for the six comic-shaped columns on that same table, which are NOT included in this universal ratification); inventory authority/state (`src/modules/inventory/`); marketplace linkage/projections (`outcome_event.channel`, generic by construction); outcomes (`outcome_event`); fees/economic components (`outcome_economics_component`); realized net (always derived, never stored); `PredictionError` (`src/lib/predictionErrorScoring.js`, pure function, never persisted).

**Universal condition, ratified definition:** universal condition = physical evidence (media) + observed defects, with grade/certification-scale interpretation as an *optional*, adapter-owned layer on top — never a kernel requirement. A numeric comic grade is NOT a universal requirement anywhere in the ratified kernel scope above; see the grade-leak findings immediately below for the two places a comic-shaped assumption currently leaks into an otherwise-universal table, both nullable and bypassable.

**Known, disclosed leaks in the ratified scope (not fixed by this ratification):** `valuation_event.grade_assumption NUMERIC(3,1)` (`db/data0/0004_data1_foundation.sql:185`) — nullable, unconstrained, not in `recordValuation`'s required-field list; a CGC-shaped numeric type sitting on an otherwise-universal table. `buyer_decision_event.observed_{title,issue,publisher,year,variant,grade}` (`db/data0/0027_buyer_decision_ledger.sql:82-87`) — six nullable free-TEXT columns, comic-shaped, embedded in an otherwise-universal economics table. Neither blocks the ratifications in this section; both are named explicitly so "universal" is not overclaimed past what the schema actually enforces.

**Three-way identity boundary, ratified:** physical identity (`gkAssetId`) is never the same thing as category/catalog identity (title, issue, publisher, and every other adapter-owned or operator-supplied descriptive attribute — today carried in `collection_item.attributes` JSONB and, for comics specifically, the pricing pipeline's own `out.*` fields) is never the same thing as external marketplace identity (an eBay item ID, a future Whatnot/Shopify listing ID — carried, per Law 1 and ADR-ID-001, as evidence/`external_map`-class relationships, never adopted as `gkAssetId`). This is a restatement and extension of Law 1 above ("Physical instance ≠ catalog class") making the third leg (marketplace identity) explicit as its own, equally-distinct axis — not a new rule, a clarification of one already ratified.

**Category-extension boundary, ratified:** `collection_item.attributes` (JSONB, `db/data0/0026_collection_item.sql`) is the ratified extension point for category-specific attributes. Its own migration comment states the intent directly: "a future book/card adapter reuses this same table, never a category-specific column set." Per this ratification, `issue`/`publisher`/`printing`/`variant`/CGC-grade/slab-raw-distinction/certification-company are explicitly **NOT** to be promoted to universal required fields anywhere in the kernel (`gk_asset`, `ownership_event`, `acquisition_event`, `valuation_event`, `outcome_event`, or any other kernel table) — they remain category attributes, carried in `attributes` or in a future category-adapter-owned structure, never kernel columns.

**Generic Asset Mode bypasses `/api/grade` and `/api/enrich`, ratified as fact, not built here.** Traced directly (U1 Review Packet, item 9): `src/modules/capture/service.js`'s `captureFromScan()` has no hard requirement for any comic field — `hasValuation`/`hasDecision`/`hasAcquisition` (`src/modules/capture/mapping.js`) are all conditional, and identity degrades gracefully to `{authority:'NONE', source:'unresolved'}` when absent. Two concrete, currently-real blockers stand between this and an actual Generic Asset Mode, ratified here as documented facts for U4 to resolve — **not resolved by this ratification:**
1. `src/modules/capture/service.js:96` hardcodes `assetClass: 'comic'` in its call to `createPhysicalAsset`, not parameterized at that call site at all (even though `createPhysicalAsset` itself already accepts an override).
2. `src/components/GrailKeyOperatorPanel.jsx`'s capture button — the only live UI path to `/api/capture-scan` — is only reachable from a `CollectionDetail` view of an item that today can only exist via the comic scan pipeline (`gradeBlob`→`/api/grade`→`/api/enrich`→`addToCatalogue`). No UI path exists today to create a catalogue item without scanning it as a comic first.

**U4 zero-migration, ratified: PROVEN.** Grounded in the migration DDL source files and the service layer's own required-field source code (U1 Review Packet, item 8) — **not a live-database query; GK-235 (Production DB target identity unresolved) makes any live-DB-dependent claim provisional, so this evidence is deliberately migration/source-file-grounded instead:**
- `valuation_event.grade_assumption NUMERIC(3,1)` carries no `NOT NULL`, no `DEFAULT`, no `CHECK` — per the column's own definition, `db/data0/0004_data1_foundation.sql:185` — and is absent from `recordValuation`'s `requireFields` list, `src/modules/assets/service.js:826`.
- Every `buyer_decision_event.observed_{title,issue,publisher,year,variant,grade}` column is nullable `TEXT` with no `CHECK` — per the table's own definition, `db/data0/0027_buyer_decision_ledger.sql:82-87` — and none are referenced by `appendBuyerDecision`'s `requireFields`/`requireNumber` calls, `src/modules/buyer/service.js:64-72` (which require only `principalId`, `sessionId`, `decision`, and the pure economics amounts — `marketValueAmount`, `contemplatedPriceAmount`, `feePct`, `suppliesAmount`, `laborAmount`, `targetProfitAmount`).
- A Buyer Mode decision can therefore be recorded today with zero comic identity fields present, per the source code's own required-field list, independent of whatever value or absence is actually stored in any live row.

**No migration is ratified or required for U4.** Migrations remain reserved for a genuine future consumer (e.g., a non-comic Buyer Mode identity snapshot needing its own generic sibling column) — not created merely because a comic-shaped column exists today, per this ratification's own governing principle. (Note, added at the GK-241 hotfix correction pass, 2026-09-21: this same migration file's `market_standing` column DOES carry a live `CHECK` constraint, `db/data0/0027_buyer_decision_ledger.sql:114`, unrelated to the comic-identity columns this ratification concerns — see GK-241/GK-243 for that separate, since-discovered finding, which does not affect the zero-migration conclusion above.)

**`asset_class` controlled vocabulary, ratified.** `gk_asset.asset_class` is a controlled vocabulary, not arbitrary free text, effective from this ratification forward. Minimum initial values: `'comic'` (existing), `'generic'` (new — the ratified default for an adapter-absent physical asset captured under Generic Asset Mode, U4). Future values are added only through an explicit, separately ratified category/adaptation decision (matching this same document's own precedent: a new Foundation Law status change requires "an explicit later audit ruling, cited by date and dispatch," per the Status governance section above — the same discipline now extends to `asset_class` values). Operator-facing UI labels may be friendly/free text; the durable `asset_class` column value itself must stay canonical/normalized. Per this ruling, ad hoc durable values such as `'Tool'`/`'tool'`/`'tools'`/`'power tool'` are explicitly NOT to be written as independent `asset_class` values — this is a governance ruling only; no code or migration enforcing it (e.g., a `CHECK` constraint) is added by this dispatch.

Full evidence trail for every claim in this section: `docs/U1-UNIVERSAL-CENSUS.md` (C1, C3, C6, C9, C11, C13) and its U1 Review Packet addendum (items 1, 2, 3, 8, 9).
