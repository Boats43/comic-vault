...
> - **ID-1** (this document's own original "DETERMINISM SCHEME" finding — `BIGSERIAL` cannot satisfy "drop-and-remint produces identical IDs"): **RESOLVED-BY-SUMMIT.** See `docs/adr/ADR-ID-001-permanent-identity.md`, Ruling 1 (UUIDv7 as the permanent scheme) and Ruling 2 (entity ID ≠ mint basis ≠ basis/rule version ≠ reconciliation version — the four-way distinction this pilot's single stable-hash conflated; corrected a second time in the Phase-1 final micro-correction, C2-v2, to drop version out of the mint-basis uniqueness tuple entirely).
