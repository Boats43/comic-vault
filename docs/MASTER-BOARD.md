...
| Field | Value |
|---|---|
| Status | Pricing engine live and gated behind the same void launch GO as Runtime. Durable valuation evidence (Foundation Law 5) is PARTIAL but materially advanced since D1: `comp_snapshot`/`valuation_event` are live (D3.3), and the full D5A→D5B→D5C chain (`market_observation` → `valuation_question`/`applicability` → `market_population`/`market_population_member`) is now **LIVE in `data1_dev`** (2026-09-03). GK-192 (question-level membership consistency) is CLOSED declaratively at the schema level (composite FKs), not delegated to a future writer. **TRUSTWORTHY D5 OPERATOR CHAINS 0 → 1 (2026-09-09, D5D Chain #1 dispatch).** The user granted a narrow, explicit exception to the Milestone Ten HOLD for exactly one Development-only runtime path: `api/enrich.js:12602` now calls `src/modules/valuation/` behind `D5D_RUNTIME_ENABLED` (default off) AND a hard `GRAILKEY_CATALOG_ENVIRONMENT==='development'` pin (Production/Preview D5D remain fully disabled regardless of the flag). Real first chain executed against Development, real authenticated principal (Jimmy), real owned asset (Creepy #1, `01a02d23-1acb-72e8-aae3-8f851308e9cf`), real eBay evidence — `valuation_question`/`market_population` 0→1, `applicability` 0→3, `market_observation` 23→26, all independently SQL-reconstructed (not accepted from the writer's own return value), idempotent replay proven (identical `idempotencyKey`, zero duplicate rows across all 6 tables). **This is explicitly NOT production capture.** GK-180 (production-traffic call sites) remains OPEN and unaffected by this narrow slice — production scan traffic still writes nothing durable. |
| Proof level | P3 (schema chain) / P2→**P1 for the Development-only slice** (writer now real-DB-proven AND reachable from one real, narrowly-gated request path; still P2 for Production/Preview, where it remains unreachable) |
| Owner | Engineering |
| Dependency | Milestone Ten phone proof still gates any PRODUCTION D5D runtime wiring — not a technical dependency, a standing authorization gate. Does not gate the Development-only exception granted 2026-09-09 (a separate, explicit, narrowly-scoped user authorization). |
| Exit gate | Milestone Ten PASS → separate, explicit D5D PRODUCTION runtime-wiring authorization (not granted by Milestone Ten PASS alone, per the user's own ruling, and not granted by the Development-only Chain #1 dispatch either) → `api/comps.js`/`api/enrich.js` call site behind GK-138 handler-wiring verification |
| Evidence | `docs/architecture/GRAILKEY-PHYSICAL-ASSET-PROTOCOL-v1.md`, Law 5; `db/data0/0004_data1_foundation.sql:178,184`; `db/data0/0014_d5a_market_observation.sql` (LIVE); `db/data0/0015_d1_identity_assignment_immutability.sql` (LIVE) + `db/data0/0016_d5b_valuation_question_applicability.sql` (LIVE) + `db/data0/0017_d5c_market_population.sql` (LIVE); `docs/adr/ADR-VALUATION-001-question-applicability.md`; `docs/D5B-LIVE-APPLY-REPORT.md`; `docs/D5C-MARKET-POPULATION-DESIGN-REPORT.md`; `docs/D5C-MARKET-POPULATION-LIVE-GATE-REPORT.md`; `docs/D5D-WRITER-BRIDGE-DESIGN-REPORT.md`; `src/modules/valuation/` |
| Next action | HOLD on Milestone Ten. Not blocked in the meantime: the write-payload spec, `comp_snapshot` compatibility ruling (DEFER, M1-A), and GK-138 test plan are all banked in `docs/D5D-WRITER-BRIDGE-DESIGN-REPORT.md` for whichever dispatch eventually performs the runtime wiring |
...
**A3 update (live-apply gate dispatch, 2026-09-03, REAL MEASURED, superseding the order-of-magnitude hypothesis above where they conflict):** `pg_current_wal_lsn()`/`pg_wal_lsn_diff()`, steady-state median of 3 samples per N, after 20-row priming, no-write control sampled, real isolated Postgres scratch schema (`tests/d5b-0015-migration-contract.test.js`, full method in `docs/D5B-LIVE-APPLY-REPORT.md`). Applicability WAL: **~990–1140 bytes/row** across N=20/60/100 and a second complete evaluation. Second-evaluation finding: a rule-version bump against the same observation/question re-evaluates as **300 additional rows (3 replicate N=100 samples), 0 deduped** — confirms the "not 1:1, re-evaluation multiplies volume" structural finding above with a real number, not merely a projection.

**Fourth volume finding — MarketPopulation (D5C design pass, 2026-09-03, REAL MEASURED where practical, hypothesis disclosed where not):** `market_population` header rows measure ~757–1168 bytes/row WAL (same method, same order of magnitude as Applicability); `market_population_member` rows measure ~768–830 bytes/row as an **upper bound** (the measurement window includes each sample's own freshly-created FK-target rows, disclosed rather than presented as an isolated marginal cost — `docs/D5C-MARKET-POPULATION-DESIGN-REPORT.md`, Deliverable H). Per-scan multipliers (rows/1K/10K/100K scans) remain **HYPOTHESIS only** — no real production corpus exists to measure against, since GK-180 confirms zero production writes anywhere in this substrate. This is now the THIRD durable table family (after `comp_snapshot` and `MarketObservation`/`Applicability`) contributing to this same pre-D6 gate — retention/recovery sizing must account for all three before D6/prod-volume release.
