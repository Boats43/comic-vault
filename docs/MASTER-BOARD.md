...
**Phase A — PROOF COMPLETE, including a same-asset integrity correction (2026-09-02).** Migration `0013_d4_identifier_fabric.sql` + exact rollback proposed; an adversarial attack against the first committed bytes found a real hole (evidence links and supersession could both cross physical assets) — fixed declaratively via composite FKs (ADR Ruling 21), not a trigger. Real isolated-scratch-schema proof against the final bytes: 77/77 (`tests/d4-identifier-fabric-migration-contract.test.js`) + 7/7 concurrency proof re-run fresh against the changed lock geometry (`tests/d4-identifier-fabric-concurrency.test.js`, `bothCommitted=false`/`cycleFormed=false` held, resolved via a real PostgreSQL deadlock this run). `data1_dev` untouched throughout; full detail `docs/DATABASE-MIGRATION-STATUS.md`, "D4 Phase A." **Phase B (applying `0013` to `data1_dev`) remains a separate, not-yet-authorized step.** No application wiring. Milestone Ten unaffected, remains OPEN / PHYSICAL-CROSS-DEVICE-PENDING.
