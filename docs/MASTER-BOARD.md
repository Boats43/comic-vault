...
| Owner | Engineering |
| Dependency | D5C `MarketPopulation` schema is DESIGNED + SCRATCH-PROVEN, NOT live-applied (`docs/D5C-MARKET-POPULATION-DESIGN-REPORT.md`, 2026-09-03) — its own live-apply dispatch is the next dependency; GK-180 (writer bridge from `api/comps.js`) remains open regardless |
| Exit gate | D5C `0017` live apply → D3.3 bridge (M1: `comp_snapshot` as a future materialized projection, not built) → D5D controlled provider-capture writer, per the A4 sequencing ratified in Phase A (this file, "Phase A — PASS") |
| Evidence | `docs/architecture/GRAILKEY-PHYSICAL-ASSET-PROTOCOL-v1.md`, Law 5; `db/data0/0004_data1_foundation.sql:178,184`; `db/data0/0014_d5a_market_observation.sql` (LIVE); `db/data0/0015_d1_identity_assignment_immutability.sql` (LIVE) + `db/data0/0016_d5b_valuation_question_applicability.sql` (LIVE); `db/data0/0017_d5c_market_population.sql` (PROPOSED, scratch-proven, NOT applied); `docs/adr/ADR-VALUATION-001-question-applicability.md`; `docs/D5B-0015-DESIGN-REPORT.md`; `docs/D5B-LIVE-APPLY-REPORT.md`; `docs/D5C-MARKET-POPULATION-DESIGN-REPORT.md` |
| Next action | D5C (`0017`) live-apply dispatch, or D5D writer-layer design against the already-live 0015/0016 substrate |
