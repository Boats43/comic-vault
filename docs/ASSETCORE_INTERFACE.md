...
**Reconciliation note (U3, 2026-09-22 — ComicAdapter/AssetCore Contract Reconciliation, `docs/adr/ADR-ADAPTER-001-adapter-contract.md` Amendment A4, `docs/TICKET-REGISTRY.md` GK-245).** This document is a **Step 4 design plan**, not a live-status report — read it as intent, not as current fact. Re-verified directly against source this pass: the "Step 5 Refactor Summary" at the bottom of this file (18 TODOs, including "ComicAdapter sets universal flags... `identityComplete`... before calling AssetCore") was only **partially executed**. Confirmed real and live today: 4 of the plan's target functions (`detectKeyValue`, `verifyStory`, `computeEraRisk`, `sanitizeComicTitle`) plus one more the plan doesn't separately name (`cleanTitleForComicVine`) — all called from `api/enrich.js`. **Confirmed NOT executed as planned:** `identityComplete` was never moved into `ComicAdapter.js` — it is computed inline in `api/enrich.js:12104-12123` (`out.identityComplete = out.assetType === 'book' ? !!(out.title && out.author) : !!out.issue`), and `src/lib/decisionEngine.js`'s own comment claiming otherwise ("identityComplete flag computed by ComicAdapter") is stale, a direct leftover of this plan's own unfinished intent rather than a description of what actually shipped. `compPoolContaminated`/`filterBypassDetected` (also named in this plan's Step 5 summary) were not independently re-audited this pass — flagged as open, not confirmed either way. The current, reconciled, evidence-cited status of every ComicAdapter call site (caller, inputs, outputs, live-vs-planned) lives in ADR-ADAPTER-001 Amendment A4, Ruling 42 — that document, not this one, is now the authoritative current-state reference; this file remains the historical record of the original Step 4/5 plan and is not rewritten by this reconciliation.
