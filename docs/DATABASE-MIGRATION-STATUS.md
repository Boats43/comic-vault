...
**Disclosed gap:** a genuine, unforced, database-generated `40P01` was **not** captured through the real service layer specifically against live `data1_dev` today, despite substantial good-faith effort (8 single-pair + 30 + 30 = 68 real concurrent trials across two concurrency tiers). The same exact trigger logic (byte-identical) **did** produce real `40P01`s twice earlier in this dispatch train, against isolated scratch schemas (Phase A). The retry algorithm is fully proven correct at the unit level (13/13, deterministic). What remains unconfirmed today is specifically the live, real-timing, through-the-actual-service combination. Not faked, not papered over — reported as a named, bounded gap.
