...
### Ruling 21 — same-asset integrity, adversarially found and closed declaratively

**A real defect, not a hypothetical one.** An adversarial attack against the originally-committed `0013` (SHA-256 `e522de5097fec2e72bea5d45dd96586b42068ab8bf6e5664ca3dd7160a5b5e4d`), run against a real isolated scratch schema before any fix was drafted, proved that **both** an evidence link across two different physical assets **and** a supersession edge across two different physical assets succeeded — no constraint stopped either. Recorded here as FACT, not silently corrected: separate valid FKs (assertion exists, observation exists; correcting target exists) do not imply the two rows share a physical asset. Nothing about `identifier_id` reuse, scope, or resolution authority was involved in either attack — the missing invariant was purely `asset_id` equality across a relationship.

**Fixed declaratively, not with a new trigger — preferred over a trigger for two reasons: it is checkable by Postgres's own planner without any procedural code, and it adds no new `plpgsql` surface for the concurrency-safe supersession trigger to interact with.**

- `asset_identifier_assertion` and `asset_raw_observation` each gained `UNIQUE (id, asset_id)` — no new uniqueness semantics beyond what the existing `PRIMARY KEY (id)` already implies; this exists solely so a composite FK elsewhere can target the pair.
- `asset_identifier_assertion.superseded_by`'s FK became a composite self-reference: `FOREIGN KEY (superseded_by, asset_id) REFERENCES asset_identifier_assertion (id, asset_id)`. No new column — `asset_id` already existed on every row. This forces a correcting target to belong to the same physical asset as the row being superseded, and says nothing about `identifier_id` — a wrong identifier may still be legitimately corrected by asserting a *different* identifier on the same asset, exactly per the standing ruling that the invariant is same physical asset, never same external identifier.
- `asset_identifier_assertion_evidence` gained an `asset_id` column with two composite FKs, one to each side. Since both FKs constrain the *same* `asset_id` value on the evidence row, they transitively force `assertion.asset_id = evidence.asset_id = observation.asset_id`. This holds for every assertion row regardless of lifecycle state — live, superseded, an intermediate chain position, or the target of multiple incoming convergent edges — proven directly against a real X→Y→Z chain, attacking both the live tail and a superseded intermediate row, both correctly rejected.

**Cost, stated exactly:** additional storage is one `UUID` column on the evidence table plus two small composite unique indexes (`(id, asset_id)` on each of the two referenced tables) — no new table, no new trigger function. Additional write cost is Postgres's own standard FK-existence check on two columns instead of one per constraint — the same class of check already paid for the original single-column FKs, not a new category of cost. **Locking:** Postgres's FK-existence checking itself acquires a `FOR KEY SHARE` lock on the referenced row, independent of and in addition to the supersession trigger's own explicit `SELECT ... FOR UPDATE` (Ruling 19) — a real additional locking surface, not assumed away. The full two-connection concurrency proof was re-run against the changed schema rather than assumed still valid: the required invariant (`bothCommitted=false`, `cycleFormed=false`) held, and this specific re-run resolved via a genuine PostgreSQL deadlock (`40P01`, one transaction aborted, one committed) rather than the plain block observed pre-fix — both are valid, previously-documented manifestations of the same guarantee (Ruling 19), and the mechanism that actually occurs on a given run is not guaranteed to be the same one twice.

**Multiplicity preserved, verified, not merely asserted:** this fix adds no `UNIQUE(asset_id)` and no `UNIQUE(identifier_id)` anywhere — one `PRODUCT_CLASS` identifier asserted on two different physical assets, and one physical asset carrying two identifiers at different scopes simultaneously, both re-proven live after this change.

New SHA-256 (final, proposed bytes): forward `f0e849228c462de953b51fa349a30231423eded58f8834b915ec9694f69d621a`, rollback `c14fa305cd7a6b48b900c28116516b1c157c0ef5fcd3226e33d6198f514774eb`.
