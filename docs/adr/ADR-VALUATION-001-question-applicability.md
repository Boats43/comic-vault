...
**ValuationQuestion** — immutable; `asset_id NOT NULL FK → gk_asset(id)` (R1); vertical-neutral semantic valuation assumptions only, admitted per R2's table above; identity reference where necessary rather than copied comic metadata; deterministic/versioned content hash (R5); no valuation result; no provider/filter/model implementation state (R3).

**Applicability** — one `MarketObservation`; one `ValuationQuestion`; verdict (R6); generic reason/rule identifier; judgment/filter/model provenance (where `COMP_FILTER_VERSION`/`modelVersion` live, R3); own recorded time; append-only correction/supersession; no durable rejection sampling cap (A).

**Not in D5B:** `MarketPopulation`; `ValuationEvent` changes; a writer from `api/comps.js`; provider capture; backfill of discarded historical judgments; 0015 schema text itself; runtime wiring.
...
- **GK-183** — unchanged by this ADR; grade_basis ambiguity is a `MarketObservation`-layer finding, not an Applicability/ValuationQuestion one.
...
R1–R6 are internally consistent with each other and with D5A's prior rulings (Q18, Q19, `mo-hash-v1`). Findings A and B are banked in this document and in `docs/MASTER-BOARD.md`. P2a is positively resolved from repository/test evidence, cited above.

**D5B SEMANTIC CLOSURE PASS — 0015 DESIGN RELEASED.**
