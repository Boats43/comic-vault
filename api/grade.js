...
export default async function handler(req, res) {
  ...
  try {
    ...
    content.push({
      type: "text",
      text:
        'Grade this comic book. Return ONLY this JSON shape with no markdown, no commentary: { "title": string, "publisher": string, "year": string, "grade": string, "isGraded": boolean, "numericGrade": number or null, "keyIssue": string, "price": string, "priceLow": string, "priceHigh": string, "reason": string, "confidence": string, "detectedPrice": string or null }. Set isGraded to true ONLY when a CGC, CBCS, or PGX slab label is clearly visible in the image; otherwise false. Set numericGrade to the numeric grade as a number (e.g. 9.8) when visible on a slab label, otherwise null. If you see a price, bid amount, current bid, Buy It Now price, or starting price visible anywhere in the image (such as from a livestream overlay, auction listing, or price tag), return it as detectedPrice (e.g. "45.00"). Otherwise set detectedPrice to null.',
    });
