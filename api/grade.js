...
// Q140-CP dispatch (2026-07-24) — interim prompt containment for the
// condition-report ("reason") field. LAUNCH-AUDIT.md Section 10 (BLOCKER
// 6-adjacent finding) documented Vision's free-text reason field asserting
// price/demand/investment language and independent series/year/cover
// attribution that competes with — and can drift from — the structured
// identity fields and the pricing engine's own computed numbers. This is
// prompt-only containment, not a schema change: it constrains what Vision
// WRITES, but the field is still free text a model can still deviate from
// under adversarial or unusual imagery. Audit label, per instruction:
...
const STANDARD_PROMPT =
  `Grade this comic book. Return ONLY this JSON shape with no markdown, no commentary: ${JSON_SHAPE}. assetTypeConfident: set to false only for a genuine non-comic object — a statue, a toy, an unrelated object, an interior page shown without any cover, or an art sheet/poster at a distinctly non-periodical size or proportion (see below). Set this to false REGARDLESS of how confident you otherwise feel about that determination — confidence describes how sure you are about grade/identity of a comic assumed to be in frame; assetTypeConfident describes whether this is a comic in the first place, and takes priority over confidence. A standard US comic cover is a tall, narrow periodical, roughly 6.6 x 10.2 inches; a poster, lithograph, or art print is typically a different, often larger, more square, or landscape proportion. A textless or minimally-texted cover at comic-cover proportions showing comic-style character art is a comic by default — set assetTypeConfident=true even when no masthead, price box, barcode, or issue number is visible. A virgin, sketch, or blank-cover variant is a real, common printing type that intentionally omits all of that text as a deliberate design choice, and a flat, straight-on cover photograph normally cannot show staples, a spine, or interior page edges regardless of whether the object is a genuine comic — their absence is the ordinary case for a cover scan, never evidence against it being one. When staples along the spine edge, interior page edges at the trim, or other physical-book cues ARE visible, treat them as confirming evidence that strengthens the comic reading, not as a requirement. Reserve assetTypeConfident=false for the genuine non-comic cases above, or for an image whose proportions or framing clearly do not match a periodical cover. When assetTypeConfident is false, still fill every other field with your best guess (never leave title/price blank), but this flag is what the app uses to decide whether to show a price at all. confidence must always be exactly one of the three words "low", "medium", or "high" — your confidence in the identification and grade, never a sentence and never a restatement or justification of the assetTypeConfident determination above; this applies even when assetTypeConfident is false. confidence describes how legible and complete THIS image is, not a general hedge. If the title, issue number, and cover art are all clearly and unambiguously legible with no damage, glare, or obstruction, report confidence as "high" — including for an ordinary, well-known book. Reserve "low" for images that are genuinely blurry, obstructed, poorly lit, cropped, or where a specific field is actually illegible or absent — never as routine caution independent of what the image actually shows. title is the series name WITHOUT the issue number (e.g. "Amazing Spider-Man" not "Amazing Spider-Man #300"). CRITICAL: Return ONLY the comic series name in the title field. Do NOT include: grade labels (VG/FN/CGC/CBCS), years, publishers, condition descriptors, seller notes, format markers, or any other metadata. Extract just the series name from the masthead. issue is the issue number as a string (e.g. "300"). If no printed issue number is visible anywhere on this cover, return issue as null. Do NOT assume "1" because you recognize the character or series — a recognizable character does not mean this specific copy is issue #1. For CGC-graded slabs: read the issue number from the CGC label header text only, not from cover art, ratio markers, or variant codes. The CGC label prints the correct issue number explicitly. year: Read the publication year directly from the cover price box, indicia, or copyright notice. Books from 2025 or 2026: return "2025" or "2026" — never default to 2024 for recent books. Read it from the cover — do not guess. If no year is visible in a price box, indicia, or copyright notice, return year as null. Do NOT infer a year from art style or character recognition — a franchise's launch era is not evidence of THIS specific copy's publication year. foreignEdition: Set to true if you see EXPLICIT visual evidence of a foreign edition: price in pence (e.g. "15p", "20p UK"), non-US currency (Canadian cents different from US, foreign publisher indicia visible on cover or copyright), or foreign-language title text. Set to false for standard US editions. When uncertain, set to false — only mark true when clear foreign indicators are present. isReprint: true if THIS SPECIFIC physical copy is itself a reprint, facsimile, or later printing — not an original first printing. Look for EXPLICIT indicators only: a "FACSIMILE EDITION" banner or text, "REPRINT" text, indicia stating a later print date than the story's original publication, or a modern reprint imprint/logo (e.g. Marvel Facsimile Edition branding). Do NOT infer isReprint from cover-art recognition alone — many facsimiles reproduce the original cover art exactly; only set true when you see explicit reprint/facsimile text or markings on this physical copy. Default to false when uncertain. editionType: return "original" for a standard first-printing copy, "reprint" when explicit reprint/later-printing text is visible, "facsimile" when explicitly labeled a facsimile edition, "variant" for a variant-cover printing that is not itself a reprint. When isReprint is true, editionType must be "reprint" or "facsimile" — never "original". Set isGraded to true ONLY if ALL of the following are visible: (1) a printed CGC, CBCS, or PGX company logo, (2) a color-coded label border (blue=Universal, green=Qualified, purple=Restored, yellow=Signature Series, or similar), and (3) a printed certification number on the label. A clear poly bag with cardboard backing board, a price sticker, an inventory tag, or a handwritten note is NOT a graded slab — set isGraded to false even if the book appears to be in rigid protective packaging. When uncertain, default to false — raw-book pricing paths handle ungraded comics correctly; a false 'graded' classification causes zero comp matches and blocks pricing entirely. grade: always return both the letter grade AND numeric equivalent e.g. "VG 4.0", "GD- 1.8", "FN 6.0". Never return just a number or just letters alone. Set numericGrade to the numeric grade as a number (e.g. 9.8) when visible on a slab label, otherwise null. certNumber: if this is a CGC, CBCS, or PGX slab, extract the certification number from the label. CGC cert numbers are typically 10 digits (e.g. "1234567890"). Return null if not a slab or cert number not visible. labelType: when a CGC/CBCS/PGX slab is visible, identify the label type from its COLOR and printed text: blue label = "universal", green label = "qualified" (a defect noted on the label, e.g. missing page — trades far below Universal), purple label = "restored", yellow label = "signature" (Signature Series), CONSERVED text = "conserved". CRITICAL: a green QUALIFIED or purple RESTORED label changes the book's value by multiples — read the label color and any type word carefully. Return null when not slabbed or the label is not clearly visible. labelNotes: transcribe VERBATIM any notes text printed on the slab label (e.g. "PAGE 12 MISSING", "3/4 SPINE SPLIT", "MODERATE (C-3) RESTORATION"). Return null when no notes text is visible. If you see a price, bid amount, current bid, Buy It Now price, or starting price visible anywhere in the image (such as from a livestream overlay, auction listing, or price tag), return it as detectedPrice (e.g. "45.00"). Otherwise set detectedPrice to null. keyIssue: return a value for: first appearance of any character, origin issues, death of major character, first issue of a series (#1), last issue or final issue, classic artist significance (Jack Kirby, Steve Ditko, Kirby/Ditko art), notable historic covers. Return null for all other issues. NEVER return No, N/A, None, or negative text. variant: if this is a variant edition return a SHORT STANDARDIZED description (under 5 words) using these formats: ratio variants: "1:25 variant", "1:50 variant". Artist/character variants: "Paco Medina Thing variant", "Alex Ross painted variant". Cover letter: "Cover B", "Cover C", "Cover D". Print variants: "2nd print", "newsstand", "direct edition", "whitman variant". Price variants: "30 cent variant", "35 cent variant". Special editions: "gold foil variant", "virgin variant", "sketch variant", "blank cover variant". Classic: "gold cover", "silver foil cover". CRITICAL: Only populate variant field when you can see EXPLICIT visual evidence of variant status (cover letter designation, ratio text, price difference, foil/virgin/sketch elements visible, or visible artist credit with variant label). Do NOT infer variant status from art style or artist recognition alone. If the cover shows no explicit variant indicators, return null even if you recognize a notable artist's style. Rare variants to specifically identify: "35 cent variant" or "30 cent variant": Marvel 1976-1977 test market — price box on cover shows 35¢ or 30¢ instead of standard 25¢/35¢. "Mark Jewelers variant": small insert booklet inside the comic advertising jewelry — look for "Mark Jewelers" insert visible at spine. "Whitman variant": diamond/Whitman logo on cover instead of standard logo. "Canadian price variant": different cover price in cents Canadian. "Pence variant": British price in pence. "double cover" or "triple cover": two or three covers on one book. "printing error": miscut, inverted cover, color error, missing ink. "Type 1A variant" or "Type 1B variant": early Marvel distribution variants. Always return these specifically if detected. Return null for standard first prints (Cover A). NEVER return just "variant" alone — always include the type. Describe cover edition, printing, or art variant only. Do NOT include signing or autograph status — that is determined solely by CGC label color. An illustrated/printed artist signature in the cover art is NOT an autograph and must not appear in this field. creator: extract the main cover artist name from cover credits or signature visible on the cover (e.g. "Brett Bean", "Frank Miller", "Jim Lee"). Return the artist who DREW the cover, not the writer or interior artist. Modern books (1990+) usually have it printed on the cover bottom or near the masthead. Return null if no creator credit is visible — do NOT guess from style. restoration: if you detect any restoration (tape, color touch, trimming, added staples, spine reinforcement), describe it briefly. Return null if none detected. defectPenalty: if a significant cover defect exists (writing, tape, sticker, torn piece, water damage) return a multiplier between 0.5 and 0.9. Return null if no defects beyond normal wear. cgcPenaltyFlags: detect five specific defects that affect CGC grading. Inspect cover, spine, edges, staples, and any visible stamps. Return as a nested object with these fields. Set each detected to true ONLY when you can clearly see the defect — when uncertain, set to false. Return null for the entire cgcPenaltyFlags object only if the image is too low-quality or partial to assess any of these. (1) storeStamp = {detected, pedigreeName}: detect circular ink stamps, retailer marks, or back-of-cover stamps identifying a comic shop. EXCEPT recognized pedigree stamps (Mile High Collection / Edgar Church, Pacific Coast, White Mountain, Promise Collection, Massachusetts, San Francisco, Boston, Allentown, Bethlehem, Big Apple, Crowley, Davis Crippen / "D Copy", Circle 8, Northford, Rockford, Twin Cities, Vancouver, Winnipeg, Curator, Larson, Long Beach, Oakland) — for those, leave storeStamp.detected=false and instead populate pedigreeStamp.pedigreeName with the recognized name. storeStamp.pedigreeName should be null (it is reserved for if you ever see a pedigree-style stamp here too — typically null). (2) staplePopping = {detected, severity}: detect staples tearing through spine, rusted staples, or missing staples. severity = "severe" when staple has fully torn through cover or interior; "minor" when early-stage rust or partial pull-through. Set severity to null when detected is false. (3) polybagIndents = {detected}: detect wave pattern across cover, rectangular seal-impression marks at edges, or visible horizontal/vertical pressure lines from polybag storage. (4) cornerChips = {detected, count}: detect missing pieces of paper at corners (NOT mere blunting or rounding) larger than a fingernail clipping. count = number of corners affected (1 to 4) when detected; null when detected is false. (5) pedigreeStamp = {detected, pedigreeName}: when you see a recognized pedigree stamp from the list above, set detected=true and pedigreeName to the canonical name (e.g. "Mile High Collection"). Set both to false/null when no recognized pedigree stamp is visible.${CONDITION_REPORT_CONTAINMENT}`;

const WATCH_PROMPT =
  `Grade this comic book from a live video frame. Read the issue number DIRECTLY from the cover. Read the title DIRECTLY from the cover masthead. Do not infer or guess — only report what you see. If a price overlay is visible report as detectedPrice. Return ONLY this JSON shape with no markdown, no commentary: ${JSON_SHAPE}. assetTypeConfident: set to false only for a genuine non-comic object (statue, toy, unrelated object, an interior page with no cover, or a poster/art print at non-periodical proportions) — regardless of your confidence in that read, this takes priority over the confidence field. A standard comic cover is tall and narrow (~6.6 x 10.2 in); a poster or print is typically a different, often larger, more square, or landscape shape. A textless or minimally-texted cover at comic-cover proportions showing comic-style character art is a comic by default — set true. A virgin/sketch/blank-cover variant intentionally has no masthead, price box, barcode, or issue number, and a flat straight-on photo of a real cover normally can't show staples or a spine either, so their absence alone is never evidence against it. Visible staples, spine, or interior page edges strengthen the read when present but are not required. true for any object at comic-cover proportions showing comic-style art; false only for the genuine non-comic cases above. confidence must always be exactly one of the three words "low", "medium", or "high" — never a sentence and never a restatement of the assetTypeConfident determination above; this applies even when assetTypeConfident is false. Report "low" only when the image itself is hard to read — not as routine caution on a clear, legible cover. title is the series name WITHOUT the issue number. CRITICAL: Return ONLY the series name — no grade labels, years, publishers, or metadata. issue is the issue number as a string. If no issue number is visible, return null — never assume "1" from character recognition alone. year: read directly from the cover price box, indicia, or copyright notice; return null if not visible — never infer from art style or character recognition. Set isGraded to true ONLY if ALL of the following are visible: (1) a printed CGC, CBCS, or PGX company logo, (2) a color-coded label border (blue=Universal, green=Qualified, purple=Restored, yellow=Signature Series, or similar), and (3) a printed certification number on the label. A clear poly bag with cardboard backing board, a price sticker, an inventory tag, or a handwritten note is NOT a graded slab — set isGraded to false even if the book appears to be in rigid protective packaging. When uncertain, default to false — raw-book pricing paths handle ungraded comics correctly; a false 'graded' classification causes zero comp matches and blocks pricing entirely. grade: always return both letter grade AND numeric e.g. "VG 4.0". certNumber: extract from slab label if visible, else null. labelType: slab label color — blue="universal", green="qualified", purple="restored", yellow="signature", CONSERVED text="conserved", null if not slabbed. labelNotes: transcribe label notes text verbatim (e.g. "PAGE 12 MISSING"), null if none. keyIssue: first appearance, origin, death of major character, first issue, classic artist significance, notable covers — null for all others. NEVER return No, N/A, None. variant: standardized short description under 5 words. Ratio: "1:25 variant". Artist: "Alex Ross variant". Cover letter: "Cover B". Print: "newsstand", "2nd print". Special: "virgin variant", "gold foil variant". Null for standard Cover A. Never return just "variant" alone. restoration: describe briefly if detected, null if none. defectPenalty: 0.5-0.9 multiplier for significant cover defects, null if normal wear.${CONDITION_REPORT_CONTAINMENT}`;
...
//
// 2026-07-18 (Edge of Spider-Verse class) — this shape used to ask ONLY
// about condition/defects on the theory that eBay's image-search consensus
// already resolved identity. But eBay's consensus confidence measures
// agreement AMONG THE RETURNED SEARCH RESULTS' titles/issues/years — it has
// zero relationship to whether the actual physical item in THIS photo is a
// comic, a foreign edition, or a reprint. A poster/print visually similar
// to a real variant cover (e.g. Peach Momoko art) can return a highly
// self-consistent set of genuine comic listings and still not itself be a
// comic. Full side-by-side comparison against the STANDARD_PROMPT JSON_SHAPE
// found 4 fields this lighter-weight path was silently never asking:
// assetTypeConfident (safety gate), foreignEdition (blocks pc_estimate for
// non-US pricing), isReprint + editionType (hard reprint-abort gate,
// api/enrich.js ~4236-4260). isGraded/certNumber/labelType were already
// present with identical instruction text — not part of this gap.
const GRADE_JSON_SHAPE = '{ "grade": string, "isGraded": boolean, "numericGrade": number or null, "certNumber": string or null, "labelType": "universal" or "qualified" or "restored" or "conserved" or "signature" or null, "labelNotes": string or null, "assetTypeConfident": boolean, "foreignEdition": boolean, "isReprint": boolean, "editionType": "original" or "reprint" or "facsimile" or "variant", "restoration": string or null, "defectPenalty": number or null, "cgcPenaltyFlags": { "storeStamp": { "detected": boolean, "pedigreeName": string or null }, "staplePopping": { "detected": boolean, "severity": "minor" or "severe" or null }, "polybagIndents": { "detected": boolean }, "cornerChips": { "detected": boolean, "count": number or null }, "pedigreeStamp": { "detected": boolean, "pedigreeName": string or null } } or null, "reason": string, "confidence": string, "detectedPrice": string or null }';
...
Your task: assess whether the physical item in THIS photo is actually the comic book described above, then grade it and detect defects.

Return ONLY this JSON shape with no markdown: ${GRADE_JSON_SHAPE}

grade: letter grade AND numeric e.g. "VG 4.0", "NM 9.4"
isGraded: Set isGraded to true ONLY if ALL of the following are visible: (1) a printed CGC, CBCS, or PGX company logo, (2) a color-coded label border (blue=Universal, green=Qualified, purple=Restored, yellow=Signature Series, or similar), and (3) a printed certification number on the label. A clear poly bag with cardboard backing board, a price sticker, an inventory tag, or a handwritten note is NOT a graded slab — set isGraded to false even if the book appears to be in rigid protective packaging. When uncertain, default to false — raw-book pricing paths handle ungraded comics correctly; a false 'graded' classification causes zero comp matches and blocks pricing entirely.
...
labelType: slab label color — blue="universal", green="qualified", purple="restored", yellow="signature", CONSERVED text="conserved", null if not slabbed
labelNotes: transcribe label notes text verbatim (e.g. "PAGE 12 MISSING"), null if none
assetTypeConfident: set to false if the image does NOT show an actual physical comic book — e.g. it's a poster, art print, statue, toy, page scan without a cover, unrelated object, or you cannot tell what the object in the image actually is — REGARDLESS of how confident you otherwise feel about grade/condition; this takes priority over the confidence field below. Set to true only when the image shows an actual physical comic book (raw or CGC/CBCS/PGX slabbed).
foreignEdition: true if you see EXPLICIT visual evidence of a foreign edition (price in pence, non-US currency, foreign publisher indicia, foreign-language title text), false for standard US editions, false when uncertain.
isReprint: true if THIS SPECIFIC physical copy is itself a reprint, facsimile, or later printing — look for EXPLICIT indicators only (a "FACSIMILE EDITION" banner, "REPRINT" text, a later print date in the indicia, or a modern reprint imprint/logo). Do NOT infer from cover-art recognition alone. Default to false when uncertain.
editionType: "original" for a standard first-printing copy, "reprint" when explicit reprint/later-printing text is visible, "facsimile" when explicitly labeled a facsimile edition, "variant" for a variant-cover printing that is not itself a reprint. When isReprint is true, editionType must be "reprint" or "facsimile" — never "original".
...
reason: brief condition report (2-3 sentences).${CONDITION_REPORT_CONTAINMENT}
  ...
  try {
    ...
    if (ebayResult && ebayResult.consensus && ebayResult.consensus.confidence >= 0.3) {
      ...
      const result = {
        ...
        labelType: gradeResult.labelType || null,
        labelNotes: gradeResult.labelNotes || null,
        // 2026-07-18 (Edge of Spider-Verse class) — these 4 fields close the
        // side-by-side gap vs STANDARD_PROMPT. assetTypeConfident is passed
        // through raw (not `|| false`) so an undefined/omitted response
        // never gets coerced into the blocking value — enrich.js's own
        // `assetTypeConfident !== false` default handles that safely.
        assetTypeConfident: gradeResult.assetTypeConfident,
        foreignEdition: gradeResult.foreignEdition === true,
        isReprint: gradeResult.isReprint === true,
        editionType: gradeResult.editionType || null,
