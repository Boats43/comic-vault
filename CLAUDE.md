...
- **GK-166 — Vercel Blob provisioned, M4-6 closed, real-photo capture proof pending Jimmy, 2026-08-23.** Real private Blob store connected to Preview/Production (REST API, CLI link-prompt didn't complete non-interactively). Fixed a real bug in `driver-vercel-blob.js`'s "already exists" recovery path (bogus placeholder URI passed to `head()`). M4-6 `BLOCKED-ON-PRIMARY-PROVISIONING` → `PASS` (39/39). Still OPEN pending a genuine real-comic-photo capture round-trip. Full detail: `docs/TICKET-REGISTRY.md`, "GK-166."
