...
- **Test baseline:** 178 PASS / 19 FAIL / 3 TIMEOUT / 200 total — verified against HEAD (this commit), 2026-08-14 (GrailKey Directive AE), via a full unfiltered run of every file in `tests/*.test.js` (not a named blast-radius subset). Prior stamp 177/19/3/199 (Directive AD). One new file this dispatch (`grailkey-directive-ae-cache-identity-atomicity`, 31/31); FAIL/TIMEOUT unchanged at 19/3, byte-identical file list to the prior stamp — every one of the 19 FAIL and all 3 TIMEOUT files individually cross-checked by name against this section's own documented list (Known stale test suites, priceBands, batch1-fixes, the grailkey-commit-e/f/g/v1 class, GK-89/90/91, dispatch-33's deliberate skip, and the 3 documented TIMEOUT artifacts) — all 22 accounted for, zero unexplained. Re-stamped in the SAME commit as the file addition. Note: `dispatch-42-comicvine-kill`'s own known-stale hardcoded `COMP_FILTER_VERSION` assertion (11) was updated forward to 12 in this dispatch as a direct, mechanical consequence of the `COMP_FILTER_VERSION` bump (GK-107) — not a new stale-test discovery, a maintained one; still TIMEOUT-classified under this sweep's 20s cutoff (27/27 passing confirmed with a longer timeout, unchanged from its prior documented behavior). **GK-89 caveat still stands**: its host file crashes (not a clean FAIL) whenever `src/lib/manualCorrection.js` is git-clean at run time. `src/lib/manualCorrection.js` was untouched by Directive AE, consistent with this stamp's own FAIL count. **Standing rule: any dispatch that adds or removes a `tests/*.test.js` file must re-stamp this line in the same commit.** Full history + methodology: Pattern Library.
