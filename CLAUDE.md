...
- **Test baseline:** 182 PASS / 19 FAIL / 3 TIMEOUT / 204 total — verified against HEAD (this commit), 2026-08-15 (GrailKey Directive AI), via a full unfiltered run of every file in `tests/*.test.js` (not a named blast-radius subset), run twice (before and after fixing one regression the first sweep surfaced — see below). Prior stamp 181/19/3/203 (Directive AH). One new file this dispatch (`grailkey-directive-ai-visual-first-identity.test.js`, 55/55); FAIL/TIMEOUT unchanged at 19/3, byte-identical file list to the prior stamp — but only after finding and fixing forward, in this SAME commit, a real regression the FIRST sweep surfaced: the new visual-first-issue-adoption mechanism was overriding `tests/grailkey-dispatch-26-fix4-zero-support-rescue.test.js`'s two explicit-decline control fixtures (a family the zero-support-rescue mechanism specifically evaluated and declined — weightSum too thin, or title collapsed to one cluster — must stay ESCALATE/null, not get second-guessed by a cruder fallback). Fixed by hoisting a `zeroSupportRescueDeclined` flag (identityCore.js) that the new branch now honors, the same "respect an already-considered decline" principle already applied to `isNearMissMarginDecline`/`isNearMissConflictActive`. Every one of the 19 FAIL and all 3 TIMEOUT files individually cross-checked by name against this section's own documented list (Known stale test suites, priceBands, batch1-fixes, the grailkey-commit-e/f/g/v1 class, GK-89/90/91, dispatch-33's deliberate skip, and the 3 documented TIMEOUT artifacts) — all 22 accounted for, zero unexplained. **GK-89 caveat still stands**: its host file crashes (not a clean FAIL) whenever `src/lib/manualCorrection.js` is git-clean at run time — untouched by Directive AI, consistent with this stamp's own FAIL count. **Standing rule: any dispatch that adds or removes a `tests/*.test.js` file must re-stamp this line in the same commit.** Full history + methodology: Pattern Library.
...
- **GrailKey Directive AI — visual-first identity authority, Slice 1, GK-113/114 CLOSED** (2026-08-15) — a physical-identity split-brain (`confirmedIssue` left null while `api/comps.js` independently re-parsed and priced against a specific issue anyway — Detective Comics #1107 class) and a "confidently WRONG vs honest null" false binary (Venom class: Vision's issue rejected by the raw pool, nothing filled the gap). New `src/lib/identityReconciler.js`: eligibility filter (Rule 1/C1 — lot/variation-group placeholders excluded from title-family candidacy BEFORE rank, GK-115 partial) + a Slice-1-scoped, pure, order-independent (D1/D4) evidence/reconcile API for the issue facet only. `resolveIdentity`'s new last-resort branch (`identityCore.js`) adopts the first ELIGIBLE visual row's issue only when nothing upstream resolved one — gated behind FIVE guards found regression-testing against this codebase's own existing precedents (no-new-information vs. Vision's already-rejected value; respecting `isNearMissMarginDecline`/`isNearMissConflictActive`/the Dispatch-26-Fix-4 zero-support-rescue's own explicit declines; a narrowed marketing-keyword filter excluding "variant" so cover descriptors don't get treated like "Anniversary Special/SDCC Exclusive" fluff; the SAME >=3-unique-corroborating-row floor `resolveFamilyIssueConsensus` already enforces) — Flash #139 and every other family-consensus-governed case verified byte-identical, structurally untouched. Authority demoted via the EXISTING `out.identityProvisional`/`listingHardLocked` mechanism (Q133 Slice 2) — `actionAuthority.identityStanding` reads CONFLICTED, never CONFIRMED, off this alone; a contested candidate may drive research but not READY (D3). Creator registry gap fixed alongside (GK-114): "jimenez" was a bare, unambiguous alias resolving only to Phil Jimenez despite Jorge Jimenez being comparably prominent — moved into the file's own documented ambiguous-surname policy. GK-116 found and logged, not fixed: `extractIssueCandidate`'s shared 999 issue-number cap (out of scope — 19+ dependent files); this dispatch's mechanism uses a separate, narrow, uncapped hash-only extractor instead of widening the shared one. Title/year/publisher/variant/creator facets otherwise untouched (~37 existing direct writers left in place) — explicitly Slice 1 of 3, not a full identity-pipeline rewrite. `tests/grailkey-directive-ai-visual-first-identity.test.js`, 55/55. Full detail: Pattern Library.
