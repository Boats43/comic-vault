...
- **Test baseline:** 184 PASS / 19 FAIL / 4 TIMEOUT / 207 total — verified against HEAD (this commit), 2026-08-15 (GrailKey Directive AK), via a full unfiltered run of every file in `tests/*.test.js` (not a named blast-radius subset). Prior stamp 183/19/4/206 (Directive AJ). One new file this dispatch (`grailkey-directive-ak-population-precedence.test.js`, 11/11); FAIL/TIMEOUT unchanged at 19/4, byte-identical file list to the prior stamp — but only after finding and fixing forward, in this SAME commit, TWO real regressions the sweep itself surfaced: (1) demoting every bare `mode==='adopted'` family vote broke `tests/q-trackB-commit4.3-winning-family-authority.test.js`'s Spawn #351 fixture, whose OWN legacy-mapped `'adopted'` (via `decideFieldAuthority`'s `outcome: 'provisionally-corrected'`) is a confidence-aware correction, not a bare population vote — fixed by keying the demotion on `outcome == null` rather than the mode string alone; (2) the original vision-only demotion condition broke a SECOND, different Spawn #351 fixture (`tests/q-trackB-commit4.1-spawn-visual-family-merge.test.js`) whose family-population vote genuinely agreed with `first-eligible-visual` while Vision itself was merely absent — fixed by generalizing the demotion check from "does Vision agree" to `justifiedBy.length === 1` ("does ANY independent source agree"), a more principled formulation that also required revising one of this dispatch's OWN new test controls once the more correct rule made clear a previously-assumed "still demoted" case was actually "correctly not demoted." Every one of the 19 FAIL and all 4 TIMEOUT files individually cross-checked by name against this section's own documented list — all 23 accounted for, zero unexplained. **GK-89 caveat still stands**: its host file crashes (not a clean FAIL) whenever `src/lib/manualCorrection.js` is git-clean at run time — untouched by Directive AK, consistent with this stamp's own FAIL count. **Standing rule: any dispatch that adds or removes a `tests/*.test.js` file must re-stamp this line in the same commit.** Full history + methodology: Pattern Library.
