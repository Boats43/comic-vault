...
- **Test baseline:** 177 PASS / 19 FAIL / 3 TIMEOUT / 199 total — verified against HEAD (this commit), 2026-08-14 (GrailKey Directive AD), via a full unfiltered run of every file in `tests/*.test.js` (not a named blast-radius subset). Prior stamp 176/19/3/198 (Directive AB). One new file this dispatch (`grailkey-directive-ad-identity-recovery`, 48/48); FAIL/TIMEOUT unchanged at 19/3, byte-identical file list to the prior stamp — every one of the 19 FAIL and all 3 TIMEOUT files individually cross-checked by name against this section's own documented list (Known stale test suites, priceBands, batch1-fixes, the grailkey-commit-e/f/g/v1 class, GK-89/90/91, dispatch-33's deliberate skip, and the 3 documented TIMEOUT artifacts) — all 22 accounted for, zero unexplained. Re-stamped in the SAME commit as the file addition this time (see Directive AD's own commit) — the standing rule was violated twice before this (Directive J, Directive AB; see Pattern Library's standing-rule violation record); this stamp does not repeat it. **GK-89 caveat still stands**: its host file crashes (not a clean FAIL) whenever `src/lib/manualCorrection.js` is git-clean at run time — a bare checkout can show a different raw count than a run right after a dispatch that just edited that file. `src/lib/manualCorrection.js` was untouched by Directive AD (only `src/App.jsx` changed), consistent with this stamp's own FAIL count. **Standing rule: any dispatch that adds or removes a `tests/*.test.js` file must re-stamp this line in the same commit.** Full history + methodology: Pattern Library.
