...
- **Structured KV scan-logging — PLAN ONLY, no code (GrailKey Dispatch 21, 2026-08-07).** Two frequency questions this session (GK-34 stale-threshold, commit-p rank-slot frequency) both ended unanswerable because wide-window runtime-log content pulls time out — a standing tooling gap now, not a one-off. Proposal: write one compact, versioned record per scan (issue authority, family weight, terminal block reason, pool sizes) directly to the KV layer already provisioned (`api/kv-cache.js`, Upstash Redis) — queryable by direct range read, not subject to the log tool's timeout. Storage primitive (Redis Streams vs. sorted-set index), volume/cost, retention, and scope (every scan vs. interesting-only) are open questions, not decided. Full plan: Pattern Library, "GrailKey Dispatch 21."
...
- **`api/` directory has 14 files, not the 12 this doc's function-cap section describes (found 2026-08-07, GrailKey Dispatch 18, while reconciling the KV drift below) — NOT fixed, needs verification before the count is corrected.** `ls api/` lists `cgc-lookup.js, chat.js, comps.js, delist-ebay.js, enrich.js, gocollect.js, grade.js, kv-cache.js, list-ebay.js, manage.js, mega-keys.js, pricecharting-pop.js, rate-limit.js, sold.js` — 14 entries. The Key Files > API Endpoints list and the "Vercel function cap is 12 (Hobby plan)... Current count: 12/12" claim (Architecture section) both predate `kv-cache.js` and `rate-limit.js`, neither of which is listed there despite being real, live, referenced-elsewhere-in-this-doc files (`api/rate-limit.js:12` is cited directly in the GK-34 writeup in the Pattern Library). `vercel.json` has no `functions` exclusion for either file, and `kv-cache.js` has no default HTTP export (pure helper module, per this doc's own stated rule that such files still count toward the cap regardless). Production deploys are demonstrably succeeding right now (confirmed via live runtime logs pulled this same dispatch) with both files present, which means either (a) the Hobby-plan function cap is no longer 12, (b) this project is no longer on the plan tier the "12" figure assumed, or (c) Vercel's function-counting behavior has changed since that number was last verified — not established which. Do not treat "12/12, adding a new file will fail deploy" as current until checked directly (Vercel dashboard function count, or a test deploy) — flagged here rather than silently corrected, since the actual current cap number isn't known from what this investigation could check.
