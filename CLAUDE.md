...
**WHAT IS NOT LIVE IN PRODUCTION (corrected this pass, GK-215):** the entire physical-asset kernel — `gk_asset`, `media`, `collection_item_link`, `ownership_event`, `current_owner`, `acquisition_event`, `valuation_event`, `decision_event`, `mint_event`, `entity_mint_basis`, `domain_event`, `outbox`, `asset_identity_assignment` all exist as Production TABLES (schema migrated) but hold ZERO rows. `operator_action_event` (0021), `outcome_event` (0023), `outcome_economics_component` (0025) don't exist as Production tables at all — these migrations were applied to Development only. Practical effect: even after a bootstrap capture succeeds (creates the first three), a downstream LIST/HOLD/PASS operator-action recording or outcome-tracking step would still fail in Production today without also applying 0021/0023/0025 there — a real, separate, not-yet-actioned gap, distinct from H8 itself.
...
**WHAT IS NEXT:** independently prove each H8-A precondition (see `docs/TICKET-REGISTRY.md`, GK-215 for current per-item status) → set `MILESTONE_TEN_H8_BOOTSTRAP=true` → the single authorized bootstrap capture → H8-B's two-device compare → `MILESTONE_TEN_H8_PASS=true` (bootstrap disabled) → apply 0021/0023/0025 to Production (separate, still-open gap named above) → normal Production capture unlocked → Long Box economics → the outcome-data loop at scale → DATA-0E-FULL mint → 0F shadow → 0G cutover. The AWW #16 rescan (closes GK-158/159's own comic-runtime closeout gate) is a separate, still-open item on the 90-day board.
