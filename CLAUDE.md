...
- **Test baseline:** 179 PASS / 19 FAIL / 3 TIMEOUT / 201 total — verified against HEAD (this commit), 2026-08-14 (GrailKey Directive AF), via a full unfiltered run of every file in `tests/*.test.js` (not a named blast-radius subset). Prior stamp 178/19/3/200 (Directive AE). One new file this dispatch (`grailkey-directive-af-discriminative-corroboration`, 25/25); FAIL/TIMEOUT unchanged at 19/3, byte-identical file list to the prior stamp. Note: `tests/grailkey-commit-t.test.js` (T1's `isCorroboratedIdentitySource` wiring test) hardcoded `FAMILY_OVERRIDE_DECISIONS` as a 2-element array and briefly regressed to FAIL mid-dispatch when this dispatch's own `COMP_FILTER_VERSION`-class change (a legitimate 3rd member added to that shared constant, GK-98) landed — caught by this dispatch's own regression sweep, updated forward in the same commit (same "correcting forward" convention as `dispatch-42-comicvine-kill`'s `COMP_FILTER_VERSION` assertion, Directive AE); not left in the FAIL list. Every one of the 19 FAIL and all 3 TIMEOUT files individually cross-checked by name against this section's own documented list (Known stale test suites, priceBands, batch1-fixes, the grailkey-commit-e/f/g/v1 class, GK-89/90/91, dispatch-33's deliberate skip, and the 3 documented TIMEOUT artifacts) — all 22 accounted for, zero unexplained. Re-stamped in the SAME commit as the file addition. **GK-89 caveat still stands**: its host file crashes (not a clean FAIL) whenever `src/lib/manualCorrection.js` is git-clean at run time. `src/lib/manualCorrection.js` was untouched by Directive AF, consistent with this stamp's own FAIL count. **Standing rule: any dispatch that adds or removes a `tests/*.test.js` file must re-stamp this line in the same commit.** Full history + methodology: Pattern Library.
