...
- **Test baseline:** 188 PASS / 19 FAIL / 4 TIMEOUT / 211 total — verified against HEAD (this commit), 2026-08-16 (GrailKey Directive AN), via a full unfiltered run of every file in `tests/*.test.js` (not a named blast-radius subset). Prior stamp 187/19/4/210 (Directive AM). One new file this dispatch (`grailkey-directive-an-physical-corroboration.test.js`, 11/11); FAIL/TIMEOUT unchanged at 19/4, byte-identical file list to the prior stamp (`tests/artist-registry-sync.test.js`, `batch1-fixes.test.js`, `comp-filter-hygiene.test.js`, `decision-engine.test.js`, `grailkey-commit-e/f/g.test.js`, `grailkey-commit-v1.test.js`, `grailkey-directive-j-gk79a-relabel.test.js`, `grailkey-directive-p-task3-variant-on-card.test.js`, `grailkey-directive-q-variant-null-custody.test.js`, `grailkey-dispatch-33-parity-harness.test.js`, `identity-gate.test.js`, `image-search-extraction.test.js`, `mega-keys.test.js`, `pattern-k-dedupe-issue.test.js`, `priceBands.test.js`, `q-adv397-visual-guard.test.js`, `sold-verification.test.js` FAIL; `dispatch-42-comicvine-kill.test.js`, `grailkey-commit-m-pc-query-fallback.test.js`, `grailkey-directive-aj-http-handler.test.js`, `ship26-integration.test.js` TIMEOUT) — zero new regressions from this dispatch's `src/lib/imageSearchIdentity.js` change. `q140-issue-consensus-corrective.test.js` re-run directly (124/124, file byte-identical to HEAD). Two pre-existing test files required real fixture corrections, both caught by this same full sweep, not assumed safe: `grailkey-directive-af-discriminative-corroboration.test.js` (3 fixtures reordered + 1 assertion updated + 1 fixture's premise revised, 25/25 restored) and `grailkey-directive-ag-22e-provenance-exemption.test.js` (same shared fixture pool, same reorder, 32/32 restored) — both shared a Sabrina test pool whose row ordering (generic row first) didn't match the real, independently-verified production ordering (NYCC row genuinely rank-1); relocated forward per the Q22/GK-19 precedent. **GK-89 caveat still stands** (its host file crashes, not a clean FAIL, whenever `src/lib/manualCorrection.js` is git-clean at run time — untouched by this dispatch). **Standing rule: any dispatch that adds or removes a `tests/*.test.js` file must re-stamp this line in the same commit.** Full history + methodology: Pattern Library.
