...
**SHIPMENT BASELINE — 2026-08-26 (GK-168/169/172 + E-UX train):** 229 PASS / 19 FAIL / 2 TIMEOUT / 250 total — first full 250-file sweep run for this train's baseline close-out (not predicted). Exact FAIL/TIMEOUT filename rosters, known-nondeterministic/cutoff set, and new-train test files: `docs/TEST-BASELINE-HISTORY.md`. New train tests: `tests/gk168-edition-facet.test.js` (85/85), `tests/eux-edition-verify-panel.test.js` (33/33). **The historical baseline did not contain a committed byte-exact filename roster — this shipment commit establishes that durable roster going forward.** **Roster delta (2026-09-02, D4 Phase A):** +2 files — `tests/d4-identifier-fabric-migration-contract.test.js` (67/67) and `tests/d4-identifier-fabric-concurrency.test.js` (7/7), both real isolated-scratch-schema/two-connection proofs of `db/data0/0013_d4_identifier_fabric.sql` (proposed, not applied to `data1_dev`). Roster count 250→252 going forward; the pre-existing 229/19/2/250 figures themselves were not freshly re-run this pass — `historical-roster status changes: NOT FRESHLY MEASURED`.
