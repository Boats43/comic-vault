...
- **GrailKey Dispatch 25** (2026-08-07) — unanimous-consensus identity unblock, in progress. Fix 1 STEP 1 (evidence-eligibility instrumentation, zero behavior change) SHIPPED (`b2c7358`, deployment `dpl_Hb7TwiSARLgQGV9PJJrPbWBF2KaG` verified READY by ID). Bare-title hypothesis KILLED at the code level — `TARGET_ISSUE_PROVISIONAL_AUTHORITY` rejects unconditionally, before any title/issue check runs. Fix 1 and Fix 2 confirmed as one root cause: `issueAuthority.status='provisional'` independently gates four blocks (CV/PC lookup skip, active-cache skip, evidence-eligibility elimination, `commit4-terminal` ID_REQUIRED force) — a single upstream promotion resolves all four. **Fix 2 GREENLIT** (explicit price-authority greenlight given); plan review for the `[commit4-promote]` predicate in progress, not yet coded. GK-35 opened (log only): `PRICING_GATE_CODES` covers only 6 of 14 rejection codes — 8 mismatch classes (including `WRONG_VARIANT`) have zero pricing-layer redundancy beyond the legacy comp filters.
