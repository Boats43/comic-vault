...
- **Test baseline:** 172 PASS / 19 FAIL / 3 TIMEOUT / 194 total — verified against HEAD (this commit), 2026-08-14 (GrailKey Directive U). Prior stamp was 174/16/3/193 (GrailKey Directive T). This dispatch added one new test file (`grailkey-directive-u-task2-stale-automatic-write` 33/33, exit 0) — that alone would be 175/16/3/194. The FAIL count moved 16→19 for a DIFFERENT reason: Directive U's own regression sweep (re-running every suite touching `App.jsx`'s scan/catalogue paths, not just the ones this dispatch's code touches) surfaced three previously-uncounted failures, confirmed via direct `git stash`/`git diff` comparison to be pre-existing and unrelated to this dispatch's changes (identical failure with Directive U's `src/App.jsx`/`src/lib/scanOwnership.js` edits stashed away) — logged, not fixed, per this dispatch's explicit scope: `grailkey-directive-q-variant-null-custody` (GK-89, harness crash — its internal `git stash`/`git stash pop` dance throws whenever `src/lib/manualCorrection.js` has zero uncommitted diff at run time, which is the normal state now), `grailkey-directive-p-task3-variant-on-card` (GK-90, a real but pre-existing scope-check false-alarm — Directive T's Task 3 rewrote `mergeConfirmedIdentity`, and this file's "unchanged" assertion was never updated to match), `grailkey-directive-j-gk79a-relabel` (GK-91, permanently stale by construction — the test pins its pre-fix proof to the literal string `'HEAD'` instead of a fixed SHA, so it was only ever going to pass until the next commit landed). None of these three were previously visible in this baseline's FAIL list — this re-stamp is the first time they're counted. Net: 174→172 PASS (-3 newly-discovered pre-existing fails, +1 new file), 16→19 FAIL (+3), 193→194 total (+1 new file), TIMEOUT unchanged. Code changes this dispatch: `src/lib/scanOwnership.js` (new `wasSupersededByCorrection` predicate), `src/App.jsx` (gradeBlob's three write sites + `submitManualCorrection`'s ownership tag). Every suite touching `App.jsx`'s scan/catalogue paths (35 files, full list: Pattern Library) individually re-run directly, plus `q-trackB-commit3-manual-correction` (466/0), `q-trackB-commit4-adoption-provisional` (152/0), `grailkey-directive-h-item1-stale-pc-anchor` (6/0), `slice7-scan-ownership` (15/0), and `q140-issue-consensus-corrective` (124/0, the Flash #139 regression guard, byte-identical as required) — 0 regressions attributable to this dispatch's own changes. **Standing rule: any dispatch that adds or removes a `tests/*.test.js` file must re-stamp this line in the same commit** — a stamp that silently drifts from the real file count is exactly the "157 vs 159" gap this re-stamp exists to close. This stamp is fresh as of 2026-08-14; treat it as due for re-verification after 2026-08-15 if no closer dispatch has already re-confirmed it. **Note for future re-stamps:** GK-89 in particular means a bare "clean checkout, run everything" pass will show a DIFFERENT count than a pass run immediately after a dispatch that just edited `manualCorrection.js` — this baseline's FAIL count is state-dependent for that one file until GK-89 is fixed.
...
- **GrailKey Directive 2026-08-11-G** (2026-08-11) — Authority Propagation Invariant named, generalizing "Rejection Must Not Create Authority" past rejections specifically to any computed/classified/corrected/preserved/replaced value; three distinct mechanisms under it (normalized by Directive H, Item 4 — see below): Computed-Then-Discarded (6 confirmed instances), Validation Bypass on Authority Replacement (GK-76, 1 instance), Stale Authority Inheritance (2 instances). **GK-74 SHIPPED/CLOSED** (`359f751`) — decisionEngine.js's magnitude safety checks now coerce `item.price` before comparison; **GK-75 opened** for the explicit scope limit (decision.price computation sites, computeBestChannel). **Task 1 SHIPPED** — `recommended-below-floor` no longer claims a raise occurred (none does; `responseContract.js:741` unconditionally discards decisionEngine's floor-raise attempt). **Task 2 SHIPPED** — `out.pcAnchorTrust`/`pcAnchorYear` now stamp unconditionally whenever a PC record exists (previously gated on an empty comp pool, the opposite of when it's needed); `src/lib/pcAnchorAuthority.js` gates Last Sold/Price Ladder/CGC Population to EXACT_EDITION, annotating (never hiding, I13) every weaker verdict. **GK-68 reclassified DEFERRED→OPEN** — its deferral premise (operational suppression closed the exposure) is false; the gap surfaces through the undeferred decision-warning panel. Full detail: Pattern Library.
- **GrailKey Directive 2026-08-11-H** (2026-08-11, corrective, pre-push) — caught before any push reached production. **Item 1 SHIPPED** — Task 2's `pcAnchorTrust`/`pcAnchorYear` merge inherited stale values on absence at 6 of 7 explicit `src/App.jsx` sites (`enrich.X ?? cur.X ?? null` resurrected a prior `EXACT_EDITION` verdict after a scan with no current PC match); fixed by dropping the `cur.` fallback entirely. 8th site (`buildCorrectedCatalogueItem`, full-spread merge) had the same symptom via a different mechanism — `pcAnchorYear` added to its clear-list (`pcAnchorTrust` was already there, added Commit E1 2026-07-29, before this field existed). **Item 2** — corrected Task 2's own test evidence: the original "shown failing pre-fix" proof was a moved-aside module (`ERR_MODULE_NOT_FOUND`), which proves an import breaks, not that prior render conditions violated the invariant; replaced with a literal mirror of the real pre-fix `src/App.jsx` conditions (verified against `26d5cf6~1`) run against Bone's production shape. **Item 3** — verified the Task 4 Pattern Library append for corruption; none found, confirmed clean by direct read. **Item 4** — normalized the Computed-Then-Discarded table (8→6, folding one duplicate row and demoting one non-instance to a design-gap note), added mechanism (c) Stale Authority Inheritance, widened the invariant statement to cover preservation as well as replacement. Test baseline re-stamped: 164/16/3/183 → 165/16/3/184 (one new file this dispatch, `grailkey-directive-h-item1-stale-pc-anchor`, 6/6 clean; `grailkey-dispatch-g-task2-pc-anchor-authority` grew from 7 to 10 assertions, still 0 fail — not a new file). Full detail: Pattern Library.
- **GrailKey Directive 2026-08-12-J** (2026-08-12) — Task 1: registry reconciliation — GK-77 split into GK-77A (REPRINT_RE Nth-print coverage gap, blocked on GK-77B) and GK-77B (no printing-authority signal exists for the scanned book at all); GK-78 reworded from "unsigned" to "variant-unverified" (the logs prove only zero positive signed-title matches, not confirmed-unsigned); GK-79 split into GK-79A (label mislabel) and GK-79B (gradeMultiplier has no evidence-coverage check, explicitly must NOT be gated on confidence). New standing invariant: unknown printing confers no exact-print authority. New candidate-patterns section (structurally separate from established doctrine): "Confidence Without Coverage," one observation (GK-79B), not promoted. **Task 2 SHIPPED/CLOSED (GK-79A, `1d827e7`)** — `src/App.jsx:5772` relabeled "Condition confidence" → "Image legibility confidence," display-only, 18/18 new test. **Task 3 (GK-78 pre-implementation trace, investigation only)** — VERDICT: SIGNAL DOES NOT REACH. `isActivePoolVariantConfirmed(verifiedActive, variant)` is already in scope at `priceBands.js:711` with zero new plumbing, but no equivalent signal exists for the SOLD pool's isolation outcome — `variantAdjusted`/`variantVerified` exist and do reach `priceBands.js`, but are scoped exclusively to the artist/variant-token fallback track (filters 7-8), never the signed-isolation fallback (filter 10) that actually produced NYC Outlaws' failure. GK-78 requires new state propagation, not a branch change — corrects Directive I's "three lines away" / "cheapest high-value fix" framing. **Standing same-commit re-stamp rule was violated**: the test file was added in `1d827e7` (Task 2) but the baseline re-stamp landed in a separate, later commit (`aef558a`) instead of the same one; corrected forward before the final push state, per the repo's no-amend convention. Disclosed, not folded in. Test baseline re-stamped: 165/16/3/184 → 166/16/3/185 (one new file, `grailkey-directive-j-gk79a-relabel`, 18/18 clean). Full detail: Pattern Library.
- **GrailKey Directive 2026-08-13-O SHIPPED** (2026-08-13, `api/comps.js`) — comp query ladder ordering fix, successor to Directive N1's identity-resolution trace (Defect A: Sabrina Annual Spectacular 2024 #1 / Dan Parent NYCC Foil class). The image-search attempt (n=-1, Ship #20a.6.7b.3, variant-blind by ship12/Q141 design) was pushed unconditionally first in the attempts array, ahead of the first attempt (n=0) that carries a confirmed variant; the attempt loop breaks on the first attempt with any post-filter survivor, so a broad generic-copy pool could win before the variant-bearing query ever ran. Fix is ORDER ONLY — when `confirmedVariant` exists, the already-built image-search attempt moves to run immediately after the last variant-bearing attempt (n 0-2) instead of unconditionally first; no query string changed; the existing artist-specific/tpb-aware `unshift` mechanisms (which already correctly outrank image-search for their narrower cases) are untouched and still win. No-variant/book behavior is a structural no-op (verified: image-search still wins first, unchanged). Demonstrated directly against real HEAD code both ways via a git-stash dance (not a mirrored predicate): pre-fix, the generic pool wins and attempt 0 never even runs; post-fix, attempt 0 wins carrying the variant text. 24-file targeted regression (every suite importing `api/comps.js` or exercising the attempt ladder/filter chain) byte-identical pre/post, including the two pre-existing documented-stale suites this change sits adjacent to (`comp-filter-hygiene` 4 failed, `priceBands` 7 failed — both unchanged). Two findings logged, not fixed (explicit non-goals): **GK-82** — the attempt loop's break condition (`filtered.parsed.length > 0`) has no quality/confidence floor, a single junk survivor can win an attempt. Also logged: the PC-year-anchor question (whether `confirmedYear=1997` independently defeats the corrected ordering on a live Sabrina rescan) is explicitly deferred to a future dispatch's rescan, per a documented four-cause proof chain — a generic post-fix pool alone does not confirm it. Test baseline re-stamped: 166/16/3/185 → 167/16/3/186 (one new file, `grailkey-directive-o-comp-ladder-reorder`, 5/5 clean). Full detail: Pattern Library.
- **GrailKey Directive 2026-08-13-P** (2026-08-13) — Task 1 (title-family `top-rank-guard`/weighted-consensus overlap veto, corroborated-adoption trace): **BLOCKED, no code shipped for Task 2.** Two independent findings each individually sufficient to stop: (1) `topFamily.weightSum` (`scoreTitleFamilies`, `imageSearchIdentity.js:1195-1206`, `getRankWeight`) is a direct sum of a per-rank weight table (rank 1→5, rank 2→4, rank 3→3, ...) — the "family won on weight 9.0 vs 5.0" signal the directive's proposed bypass would key on is rank authority by construction, not rank-free aggregate evidence; a bare row count (2 vs 7) flips the answer, confirming Blocker 3's own stated test. (2) `confirmedVariant` is not yet computed at the guard's decision point — `selectTitleFamilyCandidate` fires at `api/enrich.js:2867`, `confirmedVariant` isn't declared until `api/enrich.js:5348`, ~2700 lines and multiple pipeline phases later; confirmed directly against the real 21:16 production log (guard fires 21:16:07.402, `confirmedVariant` write logged 40ms later). Logged as **GK-83**. A third, non-blocking but load-bearing finding: even if both blockers were cleared, the directive's proposed "≥2 unique rows" corroboration floor would collide with the pre-existing, real-production-justified `Q38` gate (`imageSearchIdentity.js:2760`, commit `6e35aa8`, "Require ≥3 members for weighted-consensus override" — shipped specifically because a 1-member family with an internally-mismatched issue number silently overrode Vision) — any future implementation must be a structurally separate decision path with its own return, not a modification inside the existing weighted-consensus branch, or it inherits Q38's 3-member floor by falling through to it. Per Directive P's own explicit instruction, Task 2 stops here — Jimmy rules on next steps, not this dispatch. **Task 3 SHIPPED** (`src/App.jsx`, display/custody only, independent of Task 2) — two real gaps found and fixed: (a) the live fresh-scan `result` state (`gradeBlob`'s two `setResult` merges) did a bare `{...prev, ...enrich}` spread, which brings in `enrich.variantNote` under its server field name but never renames it to `result.variant` (the field `ResultCard` actually reads) — the operator's very first post-scan view stayed frozen on Vision's pre-enrich guess and never reflected enrich.js's own confirmation/correction, fixed with an explicit `variant: enrich.variantNote ?? prev.variant` rename at both sites (server-side custody itself — `out.variantNote`'s Q135 universal fallback, the 4 pre-existing catalogue-merge renames, and `mergeConfirmedIdentity`'s presence-aware merge — was already correct and untouched); (b) even where `item.variant` WAS correctly populated (`CollectionDetail`), it rendered roughly 1,400 lines below the title block instead of adjacent to it — moved to directly under the title/publisher/year header for both `ResultCard` and `CollectionDetail`, old buried duplicate removed (not left showing twice), the separate compact collection-list-tile badge deliberately left untouched. New candidate pattern recorded, single observation: **"hidden actionable identity is a product defect"** — a facet authoritative enough to shape search/filtering/pricing must be visible to the operator; not promoted to doctrine. Test baseline re-stamped, plus a carried-forward correction: Directive O's own comp-ladder test was missing the `process.exit()` call the Upstash-handle convention requires (exited 124/TIMEOUT despite 5/5 real passes) — fixed in this commit, not left as a false-TIMEOUT signal for the next sweep. 167/16/3/186 → 168/16/3/187 (one new file, `grailkey-directive-p-task3-variant-on-card`, 20/20 clean at ship time — repaired to 19/19 by GrailKey Directive Q, see that dispatch's entry below; this line describes this dispatch's own commit, not the file's current state). Full detail: Pattern Library.
- **CORRECTED (2026-08-13, GrailKey Directive Q)** — Directive P's Task 3 fix above was itself defective, and the line just above overstates it: **`??` is not presence-aware.** It falls through on both `undefined` AND an explicit `null` — so a genuine server-side revocation (`out.variantNote` sent as an own-property `null`, confirmed reachable on every normal, non-early-return `/api/enrich` response) resurrected the stale prior variant instead of clearing it, violating P's own acceptance requirement ("conflicted/unconfirmed → never rendered as confirmed"). The "4 pre-existing catalogue-merge renames... was already correct" claim above is also false — those four use `||`, which has the identical defect (plus falls through on empty string too). Only `mergeConfirmedIdentity`'s single call site was genuinely presence-aware, as claimed. A 7th, differently-shaped defect found in the same corrective audit: `buildCorrectedCatalogueItem` (`src/lib/manualCorrection.js`) unconditionally discarded the server's real variant on every manual correction (a rename gap, not a resurrection — `variant` is cleared as identity-dependent but `enrichData.variantNote` was never read to repopulate it). **8 audited merge-boundary entries under this dispatch's own accounting (the 6 `App.jsx` sites + `manualCorrection.js` + `mergeConfirmedIdentity`) / 7 defective, 1 (`mergeConfirmedIdentity`) already correct.** All 7 defective sites fixed, but **not to one uniform contract**: the 6 `App.jsx` sites are genuinely presence-aware (`Object.prototype.hasOwnProperty.call` inline — not exporting `dataQualityGuard.js`'s private `hasKey` or widening `mergeConfirmedIdentity`'s scope to fields those sites don't otherwise use — absent key → preserve prior, present key (value or null) → trust the fresh signal), while `manualCorrection.js`'s one site uses a plain `enrichData?.variantNote ?? null` — **absent there clears to null, it does not preserve prior**, because `buildCorrectedCatalogueItem` deliberately discards the old identity's variant before this line runs (no legitimate "prior" exists at that point to protect, unlike the six `App.jsx` sites). This is the **third confirmed instance of Stale Authority Inheritance** (mechanism (c) of the Authority Propagation Invariant) — promoted from candidate only after proving `variantNote: null` reaches these merges as a real own-property on the normal response path, not merely a structurally-plausible risk. Also corrected: P's own test (originally 20/20 passing) verified source structure and string placement, never a merge outcome — it stayed green while this shipped, the same "structure passing for behavior" lesson as Directive H, Item 2. That test was repaired in this same commit (two of its assertions consolidated into one broader check, now 19/19) — see the "3c" note in GrailKey Directive R's entry below for a wording inconsistency this left standing elsewhere in this file, corrected there. Test baseline re-stamped: 168/16/3/187 → 169/16/3/188 (one new file, `grailkey-directive-q-variant-null-custody`, 49/49 clean, asserts merge OUTCOMES against constructed inputs, not source text). Full detail: Pattern Library.
- **GrailKey Directive 2026-08-13-R** (2026-08-13) — Directive Q's own handoff inferred, rather than proved, that `api/enrich.js`'s two early-return paths (refused-identity exit, then ~5911; Q32 merchandise hard-gate exit, then ~7394) safely omitted `variantNote`. Traced both: **verdict (B), live defect, for both.** `confirmedVariant` (`:5348`, unconditional handler-scope `let`) is fully resolved by either return regardless of which branch fires, and has no separate "unknown" state distinct from its resolved value (string or `null`) — so the omission risk the directive worried about (an ambiguous null that could mean either "revoked" or "never established," verdict (C)) does not apply to this specific variable; whatever it holds at that point IS the verdict, safe to copy. The refused-identity exit's own `refusedOut` response object is built from `sanitizeIdentityFields(req.body)` (no `variant` field in that shape at all) plus an explicit field list that — unlike every other identity-adjacent field there (year/publisher get real treatment, comps/comicVine/priceCharting get explicit nulls) — never mentioned variant even once, an asymmetric gap relative to how carefully every other field was handled, not a deliberate design choice. The merchandise-gate exit returns `out` directly, but all three of `out.variantNote`'s real write sites sit later in the handler than this early return (confirmed by direct read). Fixed both with a guarded `if (X.variantNote === undefined) { X.variantNote = confirmedVariant || null; }`, reproducing the Q135 fallback's own contract exactly, positioned so an already-established value (none currently exists on either path, but the guard is defensive regardless) is never clobbered. Corrected three record errors in Q's own handoff/CLAUDE.md/Pattern Library entries (merge-boundary count phrasing, the false "all 7 fixed to one uniform contract" claim, a stale "20/20" reference for P's already-repaired test) — folded into the Directive Q entry above rather than left standing. `grailkey-dispatch-19-fix5-asset-type-override`, `q141c-marketplace-category-rejection`, `pipeline-audit`, `q110-intake-nonblocking`, `q131-systemic-audit-fixes`, and `q140-issue-consensus-corrective` (the Flash #139 regression guard) re-run directly: byte-identical, 0 new failures. Test baseline re-stamped: 169/16/3/188 → 170/16/3/189 (one new file, `grailkey-directive-r-early-return-variant`, 19/19 clean, extracts and evaluates the real committed guard statements — labeled MIRRORED, not a full end-to-end HTTP invocation of the handler). Both returns proved (B), both fixed and tested — **rescan cleared.** Full detail: Pattern Library.
- **GrailKey Directive 2026-08-14-T** (2026-08-14) — picker foundation, five tasks. **Task 1** — corrected GK-84's trigger: `UploadSiteHostedPictures` specifically decommissions 2026-09-30 (Media API migration target), not a general Trading API sunset; `AddFixedPriceItem`/`GetItem`/`EndItem` have no announced deadline. **Task 2 (GK-86)** — `extractIdentityFromImageSearch` (`imageSearchIdentity.js`) now captures one normalized `imageUrl` field (`image.imageUrl ?? thumbnailImages[0].imageUrl ?? null`); no consumer broke (field-access patterns only, no exact-shape assertions found across all 8 importing files). **Task 3 (GK-85) — the live-defect fix.** `identityAuthority`, a per-field `facet -> 'OPERATOR_CONFIRMED'` map, now crosses every merge boundary with the same presence-aware discipline Directive Q established for identity values themselves (`mergeIdentityAuthority`, new export, `dataQualityGuard.js`): absent preserves, present-value replaces, present-null clears. `mergeConfirmedIdentity` (the merge function shared by 5 of `App.jsx`'s identity-writing call sites) now refuses to let a fresh automatic value overwrite a locked facet — per-field, not per-item, so a locked title/issue doesn't block an unlocked year from still being filled by fresh catalog corroboration (the acceptance test this design choice hinges on). `api/enrich.js` now sets `out.identityAuthority` from `manualCorrectionRequest.validation.acceptedFields` on every valid correction; `out.manualCorrection` stays exactly what it was — a write-only historical record, deliberately not repurposed as the authority carrier. `buildCorrectedCatalogueItem` (`manualCorrection.js`) merges it via the same function (its raw `{...cleared, ...enrichData}` spread would otherwise wholesale-replace the whole map, dropping any earlier correction's locks). **Task 4** — `variant` added to `MANUAL_CORRECTION_ALLOWED_FIELDS` (now 5 fields), threaded through `NORMALIZERS`, `validateManualAuthority`, `prepareManualCorrectionRequest`'s `workingIdentity`, `buildManualCorrectionPayload`, and server-side (`confirmedVariant`'s init now checks for an accepted manual `variant` field before falling back to Vision's printing-claim-filtered read). **Task 5 (GK-87, correction-flow scope)** — `submitManualCorrection` now mints a fresh ownership identity (reusing `activeScanRef`/`scanGenerationRef`, no new plumbing) the instant a correction begins and gates its own write `SCAN_OWNERSHIP_MODE.ENFORCE`, narrowly, at this one call site — the global `CURRENT_SCAN_OWNERSHIP_MODE` (SHADOW, governing `gradeBlob`'s two existing sites) is untouched, per the explicit no-global-flip constraint. **Deliberately deferred, logged against GK-87**: `gradeBlob`'s own pre-existing automatic-scan persisted-write path still has no generation check — a separate, larger, riskier change to different live code, not rushed into this dispatch; Task 3's per-field authority already protects the specific identity-facet class of bug there regardless of race timing, narrowing the remaining exposure to un-locked fields only. Every regression suite touching the four touched files re-run directly, byte-identical; `q140-issue-consensus-corrective` byte-identical as required. Test baseline re-stamped: 170/16/3/189 → 174/16/3/193 (four new files, one repaired). Directive S's own STOP GATE (GK-85/86/87) is now closed for the scope this dispatch addressed — S resumes at its own Task 2 next, no re-trace. Full detail: Pattern Library.
- **GrailKey Directive 2026-08-14-U** (2026-08-14) — corrective, two items. **Task 1** — GK-84 was never actually closed (its registry line read OPEN throughout); the false "closed" claim lived only in `docs/PATTERN-LIBRARY.md`'s Directive T handoff prose, corrected in place, registry-only commit (`30d2e8b`) pushed before any code. **Task 2 — GK-87's remaining gap, closed.** Directive T's Task 5 gated the correction's own response but never stopped an OLDER `gradeBlob` (automatic scan) response from arriving after a correction begins and writing stale state anyway — exactly the "operator-confirmed identity beside a stale generic evidence panel" shape a picker built on top of T-only state would have produced. Fixed via `kind: 'scan' | 'correction'` tagging the ownership object gradeBlob/`submitManualCorrection` already mint (same shape, one field, not a new mechanism) and a new pure predicate `wasSupersededByCorrection(closure, active)` (`src/lib/scanOwnership.js`); gradeBlob's three write sites (grade-stage transient `setResult`, enrich-stage transient `setResult`, enrich-stage persisted `setCatalogue`+`setSelectedItem` gated as ONE unit) each compute their `applyScanOwnershipGuard` mode dynamically — correction-caused staleness always rejects, every OTHER staleness cause (scan-vs-scan included) stays exactly SHADOW, unaffected, `CURRENT_SCAN_OWNERSHIP_MODE` itself untouched (no global flip, as required). Reachability checked directly, not assumed: auto-refresh structurally excluded (`if (selectedItem) return;` blocks it for a correction's whole duration); back-to-back corrections already protected (T's own gate); the scan-to-catalogue `.then()` chain IS reachable — the case fixed. `refreshMarketData` found to be an independent, unrelated race through a wholly separate guard with zero relationship to `activeScanRef` — logged as **GK-88**, not fixed (out of scope, would need its own dedicated wiring). 33 new assertions (`grailkey-directive-u-task2-stale-automatic-write`), including a DIRECT pre-fix reproduction against the real unchanged `CURRENT_SCAN_OWNERSHIP_MODE` plus a `git show 1627d06` cross-check. Regression sweep surfaced three unrelated pre-existing failures (confirmed via git-stash comparison, not caused by this dispatch), logged as **GK-89/90/91**, not fixed. Test baseline re-stamped: 174/16/3/193 → 172/19/3/194 (one new file, +3 FAIL from newly-discovered pre-existing gaps, net -3 PASS). Full detail: Pattern Library.
