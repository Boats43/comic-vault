...
- **Test baseline:** 181 PASS / 19 FAIL / 3 TIMEOUT / 203 total — verified against HEAD (this commit), 2026-08-15 (GrailKey Directive AH), via a full unfiltered run of every file in `tests/*.test.js` (not a named blast-radius subset). Prior stamp 180/19/3/202 (Directive AG). One new file this dispatch (`grailkey-directive-ah-sold-fallback-authority`, 54/54); FAIL/TIMEOUT unchanged at 19/3, byte-identical file list to the prior stamp — but only after finding and fixing forward, in this SAME commit, a real regression the sweep itself surfaced: `grailkey-directive-ag-22e-provenance-exemption.test.js` (clean at AG-close) started failing because its own pre-fix comparison was pinned to the literal string `'HEAD'`, which correctly resolved to pre-AG content only until AG itself became HEAD — a second, independent instance of the exact GK-91 "designed-to-go-stale-by-construction" defect class, not previously connected to it; re-pinned to the real immutable SHA `7d0d434`. Every one of the 19 FAIL and all 3 TIMEOUT files individually cross-checked by name against this section's own documented list (Known stale test suites, priceBands, batch1-fixes, the grailkey-commit-e/f/g/v1 class, GK-89/90/91, dispatch-33's deliberate skip, and the 3 documented TIMEOUT artifacts) — all 22 accounted for, zero unexplained. `priceBands.test.js` (7 failures) and `sold-verification.test.js` (5 failures) additionally re-checked line-by-line against this dispatch's own `priceBands.js` diff (soldVerification.js untouched entirely) — same named failures, same counts, confirmed pre-existing and unrelated. **GK-89 caveat still stands**: its host file crashes (not a clean FAIL) whenever `src/lib/manualCorrection.js` is git-clean at run time. `src/lib/manualCorrection.js` was untouched by Directive AH, consistent with this stamp's own FAIL count. **Standing rule: any dispatch that adds or removes a `tests/*.test.js` file must re-stamp this line in the same commit.** Full history + methodology: Pattern Library.
