...
- **Test baseline:** 168 PASS / 16 FAIL / 3 TIMEOUT / 187 total / 0 NEW REGRESSIONS — verified against HEAD (this commit), 2026-08-13 (GrailKey Directive P). Prior stamp was 167/16/3/186 (GrailKey Directive O) — corrected forward first: Directive O's own `grailkey-directive-o-comp-ladder-reorder.test.js` was missing the explicit `process.exit()` call every `fetchComps`-importing test in this suite needs (the Upstash client leaves a dangling handle even when unconfigured), so it exited 124 (TIMEOUT) despite its 5 assertions passing cleanly — fixed in this dispatch (now exits 0), so no stamp correction to FAIL/TIMEOUT was needed. This dispatch added exactly one new test file (`grailkey-directive-p-task3-variant-on-card`, 20/20 assertions clean, exit 0), accounting for the entire +1/+1 delta (167→168 PASS, 186→187 total). This dispatch's only code change is display/custody-only in `src/App.jsx` (GrailKey Directive P, Task 3 — variant identity surfaced title-adjacent on the operator card, plus a custody fix so the live fresh-scan `result` state actually reflects `enrich.variantNote` instead of staying frozen on Vision's pre-enrich guess); zero `api/*.js` or `src/lib/*.js` files touched, so every identity/comp-ladder suite (including `q140-issue-consensus-corrective`, directly re-run, 124/0 unchanged) is structurally unaffected by construction, not merely re-verified. Directive P's Task 2 (title-family-adoption bypass) did **not** ship — blocked at the trace stage by two independent findings (family weight is directly rank-derived; `confirmedVariant` is not yet computed at the guard's decision point), so no identity-resolution code changed this dispatch. **Standing rule: any dispatch that adds or removes a `tests/*.test.js` file must re-stamp this line in the same commit** — a stamp that silently drifts from the real file count is exactly the "157 vs 159" gap this re-stamp exists to close. This stamp is fresh as of 2026-08-13; treat it as due for re-verification after 2026-08-14 if no closer dispatch has already re-confirmed it.
