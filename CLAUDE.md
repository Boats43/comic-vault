...
- **GrailKey Dispatch 2026-08-21 — DATA-1 readiness interrogation, GK-145/146/147** — first readiness/measurement pass for the "asset management system" product decision. Interrogation report: `docs/DATA-1-READINESS.md`. GK-145 (`collectionItemId` correlation wiring, explicitly NOT `gkAssetId`) + GK-146 (`scanLog` outcome/valuation measurement fields, explicitly does NOT make pricing reconstructable) SHIPPED, LOCAL ONLY. GK-147 (comic-domain-leakage debt) logged, report-only. `docs/DATA-0-ARCHITECTURE.md` gained §10 (the gkAssetId boundary + five-way record-class split + `operator_lock`-vs-`authority` naming). `tests/grailkey-dispatch-2026-08-21-gk145-gk146-measurement.test.js`, 27/27. Full detail: Pattern Library.
- **GrailKey Dispatch 2026-08-21 (urgent trace + fix) — GK-148 SHIPPED-PENDING PHYSICAL, GK-149 NOT CONFIRMED** — Creepy #1 scan. GK-148: publisher parenthetical ("Warren") fuzzy-matched Erik Larsen's alias at Levenshtein distance 2 via AU's own fuzzy fallback testing entire pool-row titles with no publisher-vs-creator awareness; fixed via `PUBLISHER_STOP_LIST` in `src/lib/premiumCreators.js` (~70 publishers, checked before distance is computed) — B2-extended found two MORE real collisions (Boom↔Broome, Disney↔Eisner/Bisley) beyond the reported one. `tests/grailkey-gk148-publisher-stoplist.test.js`, 12/12, verified genuinely non-vacuous (fails without the fix, passes with it). GK-149's hypothesized defect (comp-title evidence convicting the operator's book) does NOT match the code — the one live mechanism already reads Vision's own physical `reason` text, never a pool row; needs the real raw Vision string before any further work. Full detail: Pattern Library.
- **Phase 0.3 (2026-08-21) — GK-149/GK-148 CLOSED, GK-142/GK-143 CLOSED (2026-08-22, Detective Comics #1107 rescan, build `551db6b`), GK-141 trace overturned its own premise, GK-144 fails its own verification gate.** GK-149: Jimmy physically confirmed the Creepy #1 is the 1964 original — closed, not reproduced, no code change. GK-148: closed on the 02:50 rescan citation (zero Larsen, honest variant clearing, priced from the 24-sold pool). GK-143 ("jim inside Jiménez"): `sanitizeSeriesTitle`'s noise-word regex used plain ASCII `\b`, and `é` isn't `\w` in JS — "jim" (from "Jim Lee") false-matched inside "Jiménez," stripping to "...énez," byte-identical to the r5v6b production log. Fixed with Unicode-aware lookaround boundaries (`\p{L}`/`\p{N}`, `u` flag) at the one proven site only — reproduced with certainty, `tests/grailkey-gk143-title-corruption.test.js` 14/14, verified genuinely non-vacuous via `git stash` (fails 5/14 without the fix). GK-142 ("the series-core question"): new `deriveSeriesCoreQuery` (query-only projection, A5 — never touches display identity) wired into PC/CV/comps query construction, reusing `canonicalizeTitleCandidate`'s pipeline plus an unconditional creator-name strip (NFD-fold, position-mapped, length-guarded — same multibyte discipline as GK-143) and a separate query-only literal of the pricing `NO_PREMIUM` vocabulary — centralizing into one shared, imported constant was considered and declined push-gate-side (would require editing `NO_PREMIUM`'s own protected declaration site in `api/enrich.js`, no greenlight given); drift between the two lists is instead caught mechanically by `tests/grailkey-gk142-no-premium-parity.test.js`, which reads `enrich.js`'s live source and fails on any divergence. `tests/grailkey-gk142-series-core-query.test.js` (24/24) + `tests/grailkey-gk142-series-core-query-handler-smoke.test.js` (12/12, GK-138 — proves the r5v6b Detective shape end-to-end: display title keeps "Corner Box Jorge Jiménez" in full, comps query drops both and reaches a real 8-item pool). GK-141: trace disproved the "wiring gap" framing entirely — `fetchComps`'s handoff to `out.rawComps` is clean; the 4 survivors are discarded INSIDE `fetchComps` by a later eligibility/issue-verification gate, which is filter logic the T1-b greenlight explicitly excluded ("no filter changes") — not fixed, needs its own dedicated greenlight. GK-144: verification fails 3 of 4 gates (`@vercel/functions` isn't even installed) — stays synchronous per A6. Full 229-file regression sweep verified byte-identical to the 226-file baseline by direct before/after diff (not memory of the prior list) — only delta is the 3 new fully-passing files. Full detail: Pattern Library.
- **GK-152 SHIPPED-PENDING PHYSICAL — "the later pool knew, the identity gate didn't ask"** (2026-08-22) — Absolute Wonder Woman #16 Talavera virgin, real production trace (T1, verbatim Vercel logs). Vision misidentified entirely ("Dark Nights: Death Metal" #1); vision-zero-support ESCALATE correctly nulled confirmedIssue against a genuinely-ambiguous raw 20-item pool (43% family-level agreement, below the 60% bar — not a bug). The real gap: `api/comps.js`'s own, stronger filter chain later produced a unanimous 5/5 "#16" comps pool that never fed back into the issue facet — Commit B discarded a real, already-computed price and hard-locked the card. Confirmed DISTINCT from TARGET_ISSUE_PROVISIONAL_AUTHORITY/marketCustodyConflicted (GK-141's territory — a present-but-uncertain value; here `out.issueAuthority` was null, no value at all). Fix: `rescueIssueFromCompsPoolConsensus` (`identityCore.js`) — unanimity-only, row-count-floored, always `authority:'CONTESTED'`, reuses `projectIssueAuthority` (never a competing mechanism); `deriveMarketStanding`/`responseContract.js` gained the missing 4th per-facet CONTESTED floor (issue, alongside variant/year/title). `tests/gk152-handler-smoke.test.js` (11/11, GK-138), `tests/gk152-comps-issue-rescue.test.js` (11/11), `tests/gk152-market-standing-issue-contested.test.js` (6/6). Full 234-file sweep clean, zero new failures; one pre-existing count-assertion updated as a real, expected consequence (AQ's writer-site enumeration, 4→5). Full detail: `docs/TICKET-REGISTRY.md` GK-152, Pattern Library.
- **GK-153/154/155/156 — G.I. Joe #5 Kirkham 616 virgin, one dispatch train, 2026-08-22.** GK-153: 22e EXONERATED (it caught real corruption, not a false veto) — `sanitizeSeriesTitle`'s bare-word `'joe'` noise-strip (LEGACY_CREATOR_NOISE_WORDS) was destroying "G.I. Joe" itself; fixed via `COMPOUND_WHITELIST` additions (`g i joe`/`g.i. joe`/`gi joe`), same mechanism as X-Men/Captain Marvel. GK-154: a PC anchor already rejected for TITLE by `q141-a` was still supplying its YEAR unconditionally 500 lines earlier — fixed via a narrow gate at the `resolveYear` call site reusing `q141-a`'s own predicate to withhold `pcYear`, `q141-a` itself untouched, catalog year still surfaces as conflict evidence. GK-155: `extractConfirmedVariant` concatenated Vision's variant text with pool-consensus tokens it already contained ("...virgin exclusive limited"), reaching the live eBay query and cache key — fixed via token-set idempotent dedup; `independence=n/a` cleared as a short-circuit, not a bug (log reworded). GK-156 (systemic, found tracing GK-154): `eraLock`/`poolYearHint` read `r.title` (a SANITIZED series-name projection) instead of `r.rawTitle` — silently starving both signals below their own floors on every scan, not just this one; fixed field name plus added the previously-missing SKIPPED log lines. All four combined, proven via real handler: `tests/gk153-156-gijoe-handler-smoke.test.js`; per-fix units: `tests/gk153-gijoe-compound-whitelist.test.js`, `tests/gk154-year-anchor-gate.test.js`, `tests/gk155-variant-dedup.test.js`, `tests/gk156-poolyearhint-eralock-rawtitle.test.js`. Full detail: `docs/TICKET-REGISTRY.md` GK-153–156, Pattern Library.
- **GK-157/158 — the residual gap and the discarded-evidence gap, 2026-08-22.** GK-157: `sanitizeSeriesTitle`'s `COMPOUND_WHITELIST` `protectedHit` was PREFIX-ONLY — a compound sitting mid-string (e.g. a raw family-election candidate like "616 gi joe cobra commander," as opposed to a reconstruction that happens to lead with the compound) still lost "joe" to the noise-word strip; fixed to a word-boundary match anywhere in the string, prefix checked first and always winning (byte-identical for every existing fixture). Separately, `resolveIdentity`'s FAMILY_OVERRIDE_DECISIONS branch (title-family election WINS) wrote `sanitizeSeriesTitle(family.selectedTitle)` directly as `confirmedTitle`, never passing through `canonicalizeTitleCandidate`'s richer pipeline (attribution/merch/parenthetical/grade/finish-descriptor/issue/publisher-year-suffix — AV/AW) — previously reachable only via `reconcileTitleFacet`'s opposite, election-REFUSES branch. An elected family key now gets the identical canonicalizer pass (idempotent, over-strip-guarded, `[title-canon]` logged only when it actually changes something). GK-158: `evidenceEligibility.js`'s `TARGET_ISSUE_PROVISIONAL_AUTHORITY` branch was an UNCONDITIONAL blanket demotion — every row under a marketplace-only-adopted (provisional/conflicted) issue authority was pushed to reference-only regardless of whether the row's own title agreed with the contested value, discarding real evidence (e.g. GK-152's own comps-pool-consensus rescue shape). Fixed: `hasIssueNumber` is now evaluated even in this branch — an agreeing row is identity-ELIGIBLE (tagged `PROVISIONAL_ISSUE_MATCH`, may reach pricing) while a disagreeing row is still rejected outright (`WRONG_ISSUE`). `deriveMarketStanding`'s GK-152 per-facet floor (SIMILAR_ONLY whenever `issueAuthority.status==='conflicted'`) and commit4-terminal's listing lock are both untouched — this fix only decides whether a matching row may contribute to the price, never how confident the card looks. Side finding, logged not fixed: GK-35 (`PRICING_GATE_CODES` omits `WRONG_ISSUE`, newly reachable but still dormant in production behind comps.js's/soldVerification.js's own earlier issue filters) — `docs/TICKET-REGISTRY.md`. `tests/gk157-elected-title-canonicalization.test.js` (13/13), `tests/gk158-provisional-issue-match.test.js` (24/24), `tests/gk157-158-handler-smoke.test.js` (GK-138, real `/api/enrich` + real `api/comps.js:fetchComps`), plus two large pre-existing suites updated as real, expected consequences of the fix: `tests/q-trackB-commit4-adoption-provisional.test.js` (156/156), `tests/grailkey-dispatch-25-fix1-instrumentation.test.js` (25/25). Full detail: `docs/TICKET-REGISTRY.md` GK-157/158, Pattern Library.
- **GK-153/154/155/156/157 CLOSED, GK-152's eligibility half CLOSED (2026-08-22 19:46 rescan, build `6ecbc5d`)** — physical closure via a real production rescan of the same G.I. Joe #5 Kirkham book: compound-protected title intact, year 2025 (not the wrong 1987 anchor), variant deduped, priced from its own real market, and GK-157's word-boundary/canonicalizer fixes all hold. Separately, the same rescan cycle's Absolute Wonder Woman #16 Talavera AWW rescan confirms GK-152's own remaining pending-physical bar (an agreeing comp reaching pricing under the rescued issue, not just the rescue mechanism itself setting `confirmedIssue`) — agreeing comp passed, rejected=0. `docs/TICKET-REGISTRY.md` GK-152–157.
- **GK-159 — "commit4-terminal floors instead of clears," 2026-08-22.** The last contested facet still CLEARING instead of flooring: `computeIssueAuthorityContractPatch` (`src/lib/issueAuthority.js`, this codebase's "commit4-terminal") unconditionally nulled price and forced ID_REQUIRED the instant `issueAuthority.status==='conflicted'`, even with a real price already computed — every sibling contested facet (variant/year/title, and issue's own GK-152 `deriveMarketStanding` floor) already floors to SIMILAR_ONLY/REVIEW instead. Fixed, scoped to `issueConflicted` only: when a real price already exists, returns `{marketStandingFloored:true}` — touches no price field, sets no `identityConfident`/`refusedToPrice`/`listingHardLocked` — letting GK-152's own pre-existing floor and soft lock (both untouched) do the rest. `issueProvisional`/`yearOnlyProvisional`/the P1 commit-p carve-out are all unchanged. `api/enrich.js`'s real call site gained a third, distinct log branch (I13 parity with the existing two). Blocking fixture (real handler): the AWW shape commits a real price at `REVIEW`/`SIMILAR_ONLY`/`ISSUE_CONTESTED`, `listable=false`, never `ID_REQUIRED`/`REFUSED`/`PRICED`. `tests/gk159-commit4-terminal-floor.test.js` (28/28) + `tests/gk159-commit4-terminal-floor-handler-smoke.test.js` (18/18, GK-138). Two minor tickets logged report-only, not fixed: GK-160 (`visualReferenceEvidence` familyKey mislabel, AWW keyed as an unrelated pool member's title text — display/diagnostic only, no pricing effect), GK-161 (post-escalate publisher backfill assigning "BOOM! Studios" to a confirmed DC book — mechanism not traced). Full detail: `docs/TICKET-REGISTRY.md` GK-159–161, Pattern Library.
- **GK-163 — class-wide idempotency request fingerprint, CLOSED, 2026-08-23.** `attachMedia` was the only operation with request-fingerprint protection (hand-rolled). Now ONE shared law in `src/modules/assets/idempotency.js` (`computeRequestFingerprint`, `idempotency_key.request_fingerprint` column, migration `0010`, typed `IdempotencyConflictError`) covers all 10 operations. Blocking proof: 40/40 across every operation (same key+same payload → replay; same key+different payload → conflict, zero new rows). Full detail: `docs/TICKET-REGISTRY.md`, "GK-163."
- **GK-165 — S3 suite 84-vs-94 assertion drift, CLOSED, 2026-08-23.** Root-caused: no removed assertions, no disabled path, no changed counting mechanism found — most likely a documentation/math error in the original "94" claim. Nothing fixed; nothing real disappeared. Full detail: `docs/TICKET-REGISTRY.md`, "GK-165."
- **GK-166 — Vercel Blob provisioned, M4-6 closed, real-photo capture proof CLOSED, 2026-08-23.** Real private Blob store connected to Preview/Production (REST API, CLI link-prompt didn't complete non-interactively). Fixed a real bug in `driver-vercel-blob.js`'s "already exists" recovery path. M4-6 `BLOCKED-ON-PRIMARY-PROVISIONING` → `PASS` (39/39). Real photo of Creepy #1 1964 (Jimmy's own) captured end-to-end through `captureFromScan` → Blob, byte-identical proof, then a live Preview round-trip (`GET /api/asset-media` → `200`, real image bytes, SHA-256-verified). Full detail: `docs/TICKET-REGISTRY.md`, "GK-166."
- **GK-164 — DATA-1D correction pass, credential disclosure + session hardening, 2026-08-23. CLOSED.** `docs/adr/DATA-1D-AUTH-CROSS-DEVICE.md` printed the operator passphrase in cleartext, in a local commit never pushed to `origin`. Rotated via a local script that never prints the value; `GRAILKEY_SESSION_EPOCH` added to `src/modules/auth/token.js` so a disclosure can invalidate every outstanding token, not just future logins; `GRAILKEY_SESSION_SECRET` now has a 32-char strength floor; scrypt cost params (`N/r/p`) pinned explicitly in `credentials.js`. **Per Jimmy's ruling, the local branch was rewritten (`083c8ff → DATA-1D scrubbed → correction pass`) — the disclosing commit is superseded local history, never pushed; no `reflog expire`/`gc` run.** GK-165 opened (log-only) for an unrelated S3-proof-suite assertion-count drift found during re-verification. H5 (Production env-var proof) and H6 (Preview deploy) remain blocked — no authenticated Vercel CLI session in this environment; still gate any push of this lane. Full detail: `docs/PATTERN-LIBRARY.md`, "GK-164."
