...
- **Watch Mode**: Buyer tab → "👁 Watch Mode" → rear camera captures JPEG frames every 3s → `/api/grade` (Sonnet for speed/cost) → dedup by `title|issue` key → `/api/enrich` on new comic → shows Market + Net @ bid. Voice context (Web Speech API, continuous mode) and text hint input share `watchContext` state → appended as `"Seller said: {context}"` in grade prompt. Auto-bid parses `$N` from speech transcript. Android browser fallback: "Type context above instead" on SpeechRecognition failure.
- **Watch Mode model routing**: `api/grade.js` uses `claude-sonnet-4-20250514` when `body.source === 'watch'`, `claude-opus-4-6` for all other requests (camera snap, share target).
