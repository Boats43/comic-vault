...
- **Test baseline:** 169 PASS / 16 FAIL / 3 TIMEOUT / 188 total / 0 NEW REGRESSIONS — verified against HEAD (this commit), 2026-08-13 (GrailKey Directive Q, corrective). Prior stamp was 168/16/3/187 (GrailKey Directive P). This dispatch added exactly one new test file (`grailkey-directive-q-variant-null-custody`, 49/49 assertions clean, exit 0, asserts real merge OUTCOMES against constructed inputs rather than source text — the lesson from P's own 20/20-passing-while-defective test) and repaired one existing file's now-stale assertions (`grailkey-directive-p-task3-variant-on-card`: its Part 1 pinned to bare `HEAD` for a "prove the pre-fix defect" check, which silently stopped proving anything once HEAD moved past Directive P's own commit — repinned to the exact pre-P SHA `e95b9a9`; its Part 2/3 exact-string checks updated to match Q's corrected merge expressions — still 19/19 clean, no assertion-count change to this baseline since it was already counted as 1 PASS file). Net: 168→169 PASS, 187→188 total, FAIL/TIMEOUT unchanged. This dispatch's code changes are display/custody-only (`src/App.jsx`, 6 sites; `src/lib/manualCorrection.js`, 1 site) — see the "CORRECTED (2026-08-13, GrailKey Directive Q)" note above for what was wrong and what changed; zero `api/*.js` files touched. `q-trackB-commit3-manual-correction` (466 assertions), `q-trackB-commit4-adoption-provisional` (152), and `grailkey-directive-h-item1-stale-pc-anchor` (6) — every suite importing `src/lib/manualCorrection.js` — re-run directly: all byte-identical, 0 new failures. **Standing rule: any dispatch that adds or removes a `tests/*.test.js` file must re-stamp this line in the same commit** — a stamp that silently drifts from the real file count is exactly the "157 vs 159" gap this re-stamp exists to close. This stamp is fresh as of 2026-08-13; treat it as due for re-verification after 2026-08-14 if no closer dispatch has already re-confirmed it.
