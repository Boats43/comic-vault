...
- **Test baseline:** 193 PASS / 19 FAIL / 4 TIMEOUT / 216 total — verified against HEAD (this commit), 2026-08-17 (GrailKey Directive AR, GK-129/GK-130 build). Prior stamp 192/19/4/215 (Directive AQ-follow-up). One new file this dispatch (`grailkey-directive-ar-evidence-authority.test.js`, 42/42, covering both GK-129 and GK-130); no existing test files modified. FAIL/TIMEOUT unchanged at 19/4, byte-identical file list to the prior stamp (`tests/artist-registry-sync.test.js`, `batch1-fixes.test.js`, `comp-filter-hygiene.test.js`, `decision-engine.test.js`, `grailkey-commit-e/f/g.test.js`, `grailkey-commit-v1.test.js`, `grailkey-directive-j-gk79a-relabel.test.js`, `grailkey-directive-p-task3-variant-on-card.test.js`, `grailkey-directive-q-variant-null-custody.test.js`, `grailkey-dispatch-33-parity-harness.test.js`, `identity-gate.test.js`, `image-search-extraction.test.js`, `mega-keys.test.js`, `pattern-k-dedupe-issue.test.js`, `priceBands.test.js`, `q-adv397-visual-guard.test.js`, `sold-verification.test.js` FAIL; `dispatch-42-comicvine-kill.test.js`, `grailkey-commit-m-pc-query-fallback.test.js`, `grailkey-directive-aj-http-handler.test.js`, `ship26-integration.test.js` TIMEOUT) — zero new regressions from this dispatch's `src/lib/imageSearchIdentity.js`/`src/lib/actionAuthority.js`/`src/lib/responseContract.js`/`api/enrich.js` changes; every blocking control named in the directive re-run directly and confirmed byte-identical: `tests/q84-dual-axis.test.js` (17/17), `grailkey-directive-af-discriminative-corroboration.test.js` (25/25), `grailkey-directive-ag-22e-provenance-exemption.test.js` (32/32), `grailkey-directive-an-physical-corroboration.test.js` (11/11), `grailkey-directive-ao-companion-product-eligibility.test.js` (19/19), `grailkey-directive-ap-variant-unresolved-authority.test.js` (37/37), `grailkey-directive-aq-canonical-facet-authority.test.js` (32/32), `q140-issue-consensus-corrective.test.js` (124/124, Flash #139 unaffected). **GK-89 caveat still stands** (its host file crashes, not a clean FAIL, whenever `src/lib/manualCorrection.js` is git-clean at run time — untouched by this dispatch). **Standing rule: any dispatch that adds or removes a `tests/*.test.js` file must re-stamp this line in the same commit.** Full history + methodology: Pattern Library.
