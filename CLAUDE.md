...
- **GrailKey Dispatch 25** (2026-08-07) — unanimous-consensus identity unblock. Fix 1 STEP 1 (evidence-eligibility instrumentation, zero behavior change) SHIPPED (`b2c7358`, deployment `dpl_Hb7TwiSARLgQGV9PJJrPbWBF2KaG` verified READY by ID). Bare-title hypothesis KILLED at the code level. Fix 2/2b (unanimous-consensus promotion of `issueAuthority.status`, both issue and year axes, plus V4 — a status-promotion safety regression against `escalateIssueAuthorityOnConflict` found and closed in review before push) **committed locally as `81ca1d2`, full regression sweep clean, HELD FOR PUSH APPROVAL — not yet pushed/deployed.** See `docs/PATTERN-LIBRARY.md` for the full predicate, the year-axis "silence counted as dissent" correction (third wrong-population instance), and the V4 closure. Fix 3 (Vision virgin-variant prompt) remains HELD. GK-35 remains open (log only): `PRICING_GATE_CODES` covers only 6 of 14 rejection codes — 8 mismatch classes (including `WRONG_VARIANT`) have zero pricing-layer redundancy beyond the legacy comp filters.
