...
- **Test baseline:** 180 PASS / 19 FAIL / 3 TIMEOUT / 202 total — verified against HEAD (this commit), 2026-08-14 (GrailKey Directive AG), via a full unfiltered run of every file in `tests/*.test.js` (not a named blast-radius subset). Prior stamp 179/19/3/201 (Directive AF). One new file this dispatch (`grailkey-directive-ag-22e-provenance-exemption`, 32/32); FAIL/TIMEOUT unchanged at 19/3, byte-identical file list to the prior stamp. Every one of the 19 FAIL and all 3 TIMEOUT files individually cross-checked by name against this section's own documented list (Known stale test suites, priceBands, batch1-fixes, the grailkey-commit-e/f/g/v1 class, GK-89/90/91, dispatch-33's deliberate skip, and the 3 documented TIMEOUT artifacts) — all 22 accounted for, zero unexplained. `grailkey-commit-v1.test.js`'s failure additionally re-verified via `git stash`/`git stash pop` (identical failure with this dispatch's changes removed) to confirm pre-existing and unrelated, not just cross-checked by name. Re-stamped in the SAME commit as the file addition. **GK-89 caveat still stands**: its host file crashes (not a clean FAIL) whenever `src/lib/manualCorrection.js` is git-clean at run time. `src/lib/manualCorrection.js` was untouched by Directive AG, consistent with this stamp's own FAIL count. **Standing rule: any dispatch that adds or removes a `tests/*.test.js` file must re-stamp this line in the same commit.** Full history + methodology: Pattern Library.
