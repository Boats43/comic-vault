...
- **GrailKey Dispatch 32** (2026-08-08) — batch review of 47 real scans, STEP A source-verification pass on all 8 reported defects. Two retracted after verification, recorded together as one review-discipline note: recommended-price divergence (Spidey Super Stories #23) was the single-comp ask-based floor guard firing correctly, not an untracked variable — same failure mode (partial log excerpt read without its reconciling line) as the earlier key-multiplier retraction. One downgraded to log-only: the tokenizer-truncation lines were `22e-force`'s own FAIL output — already caught and reverted before shipping, confirmed cosmetic. New named class shipped to Pattern Library: Q84's absolute-floor coherent-content lane and `22e-force`'s percentage-floor rejection disagree by construction below a family-size crossover point (3 supporters is already ≥60% at family size ≤5) — provable from the arithmetic, not just observed from failures. Three fixes scoped STEP B (report only, no code, each needs its own greenlight): 32-A (Q84 threshold-shape + guard-before-first-consumer ordering + co-title gating, one fix, three required parts), 32-B (thread `variantIdentitySource` into `verifySoldComps`' `ctx`, mirroring the existing `TARGET_ISSUE_PROVISIONAL_AUTHORITY` issue-axis precedent), 32-C (Fix 3b `pcProductId`/convergence OR-arm for `visionLowButCorroborated`, plus a `scanLog` v2 schema gap it exposed). Multi-book lot titles (Defect 8) confirmed to have no identity-level detection anywhere in the pipeline — logged, unscoped.
