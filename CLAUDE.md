...
- **Test baseline:** 195 PASS / 19 FAIL / 4 TIMEOUT / 218 total — HEAD (this commit), 2026-08-17 (Directive AT, GK-135). Prior 194/19/4/217 (AS). New file `grailkey-directive-at-year-evidence.test.js` (31/31); one existing file corrected (`grailkey-directive-am-variant-custody-real.test.js`, a hardcoded `selectFirstEligibleVisual(parsedVisualRows)` count of exactly 2 floored to `>=2` — this dispatch's own year-evidence block legitimately adds a 3rd correct call site — 28/28 restored). FAIL/TIMEOUT byte-identical to prior stamp (full file list: Pattern Library "GrailKey Directive AR" entry) — zero new regressions; identity-gate/comp-filter-hygiene's own pre-existing failures reconfirmed unrelated (author-field/sanitizer mismatches, not year-authority). **Standing rule: re-stamp this line in the same commit as any test-file add/remove.** Full history: Pattern Library "GrailKey Directive AT."
