...
- **Test baseline:** 197 PASS / 19 FAIL / 4 TIMEOUT / 220 total — HEAD (this commit), 2026-08-19 (Directive AU fix-forward, GK-136/GK-137/GK-138). Prior 196/19/4/219 (AU first push, `1d23069` — which crashed production; see GK-138). New file `grailkey-directive-au-handler-smoke.test.js` (8/8) — a real `import handler from '../api/enrich.js'` invocation with a stubbed `fetch` reproducing the exact production ghmn7 pool, proving no `ReferenceError`, HTTP 200, and the `[reconcile-variant]` third-entry-path evidence live in the real handler, not just the library (`grailkey-directive-au-dellotto-1963.test.js`, unchanged, still 24/24 — that file never caught the wiring bug because it never exercised the handler). FAIL/TIMEOUT byte-identical to the AT-era baseline, diffed programmatically — zero regressions from the scope-hoist fix. **Standing rule: re-stamp this line in the same commit as any test-file add/remove.** Full history: Pattern Library "GrailKey Directive AU."
