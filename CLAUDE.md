...
- **Test baseline:** 194 PASS / 19 FAIL / 4 TIMEOUT / 217 total — HEAD (this commit), 2026-08-17 (Directive AS, GK-132/GK-126). Prior 193/19/4/216 (AR). New file `grailkey-directive-as-candidate-always-enters.test.js` (31/31). FAIL/TIMEOUT byte-identical to prior stamp (full file list: Pattern Library "GrailKey Directive AR" entry, unchanged since) — zero new regressions; every named control (AI 55/55, AJ http-handler 13/13, q131 Eternus 43/43, AK 11/11, q-trackB-commit4.3 263/263, q-vision-zero-support 40/40, q140 124/124) re-run byte-identical. **Standing rule: re-stamp this line in the same commit as any test-file add/remove.** Full history: Pattern Library "GrailKey Directive AS."
...
- **GrailKey Directive AQ-follow-up (build) — GK-128 CLOSED, evidence-set completeness** (2026-08-16) — greenlit build on top of the trace. Rule installed: every materially asserted issue value in an eligible conflicting family must reach the issue evidence set — a plurality winner is not a substitute for its dissent. Fix: the near-miss branch (`src/lib/identityCore.js`) now carries the runner-up title-family's own asserted-issue set forward; every runner-up value that doesn't match the preserved value is fed into the evidence set as real conflict evidence (provenance `'family-runnerup-dissent'`), scoped to the runner-up's own dissent only (never the top family's internal minority — verified required, feeding top-family dissent would resurrect GK-127 on Wolverine #90). Boundary-test chain proves both PRE-FIX (READY/listable=true) and POST-FIX (CONTESTED, conflicts=[300], CV/PC blocked, LOCKED, listable=false) on the real Batman #213/#300 fixture. Item 2 (null-defaults-to-trusted audit): traced, no additional live gap found — two candidate shapes already safely handled by pre-existing mechanisms. `tests/grailkey-directive-aq-followup-gk128-evidence-completeness.test.js`, 23/23. P0 SHIPPED-PENDING PHYSICAL — halt still active pending three physical rescans (Wolverine #90, a genuine near-miss book, Dell'Otto ASM #1). Committed locally on `e7c0eac`'s checkpoint, not pushed.
- **GrailKey Directive AR — GK-129/GK-130 SHIPPED-PENDING PHYSICAL** (2026-08-17) — one law, two tickets: authority is earned from evidence, never granted by list membership, never retained by an unresolved contest. **GK-130 (Part A):** `applyDualAxisGate` (`src/lib/imageSearchIdentity.js`) vetoed a correctly-identified 10-member family (Absolute Batman #19, Ben Oliver) purely because the cover artist wasn't in `ARTIST_PATTERNS`; `findPhysicallyCorroboratedTokens` now grants a token the same standing when it's present on the frozen rank-1 row (`selectFirstEligibleVisual`) AND named by ≥3 independent (unique-seller) family members. **GK-129 (Part B):** a CONTESTED variant facet (`reconcileVariant`) still let `deriveMarketStanding` reach EXACT_CURRENT/READY (Venom Mayhew, $48.86) because `out.variantApplicability` custody never checked reconciler authority; `api/enrich.js` now custodies CONTESTED into `variantApplicability`, floored to SIMILAR_ONLY by `actionAuthority.js` (`VARIANT_CONTESTED_EDITION`). Sixth false-READY sibling closed. `tests/grailkey-directive-ar-evidence-authority.test.js`, 42/42; full suite re-run clean. Full detail: Pattern Library, "GrailKey Directive AR."
- **GrailKey Directive AS — GK-132/GK-126 SHIPPED-PENDING PHYSICAL** (2026-08-17) — "the candidate always enters." Venom Mayhew ID_REQUIRED'd despite the frozen rank-1 row naming the exact book, because `MINIMUM_CORROBORATING_ROWS` (identityReconciler.js) gated ENTRY into the issue evidence set (not just consensus override) and `hasContaminatedMember` flagged a lone Signed/Remarked member as "contaminated" (mixture is impossible at n=1). `identityCore.js`'s unconditional issue-evidence builder: floor removed as an entry gate (kept diagnostic-only); Guard 6 now requires ≥2 members before the mixture check runs; three NEW narrow guards added after regression-testing the full suite (own-row REPRINT_RE/TPB check — True Believers class; deference to a genuine prior 'no-consensus' verdict, scoped to non-override decisions with real `assertedIssues`, excluding the 999-issue-cap false-negative Detective #1107 hits; pool-wide Vision-title-overlap check — Quux/CONTROL E class). `tests/grailkey-directive-as-candidate-always-enters.test.js`, 31/31; full suite re-run clean, zero regressions. Full detail: Pattern Library, "GrailKey Directive AS."
- **GrailKey Directive AQ — GK-127 SHIPPED-PENDING PHYSICAL, GK-128 OPEN (fifth false-READY, live)** (2026-08-16) — canonical facet authority. `reconcileIssue` (Slice 1) already correctly computed CORROBORATED for Wolverine #90's unanimous evidence; a separate, older, parallel authority system (Commit 3/4/4.1/4.3) independently re-derived `out.issueAuthority` from `familyIssueConsensus`'s mode/outcome flags and overwrote it with 'conflicted'. A plurality-only `axisAgreement` rewrite was drafted, found to regress a real Fix-2c safety property (a live dissenting row must still flag), and reverted — the real fix needed no change to that unanimity test at all. Normal visual-resolution custody now projects issue authority once from `reconciledIssue` (`projectIssueAuthority`, `src/lib/issueAuthority.js`); the YEAR-axis cross-facet write ("Wolverine Revenge") closed; operator corrections now enter evidence at maximum weight (`source='user'`). **Corrected same-day**: this is improved custody, not completed single-writer custody — three separately-scoped exceptional mutation paths remain (`escalateIssueAuthorityOnConflict`, manual-correction provenance, `checkCrossPopulationPromotionGuard`), tracked as Slice 2 work-order items. **GK-128, proven live via a real end-to-end `assembleContract` run** (Batman-#213-class near-miss: runner-up population 213/213/300, the "300" dissent never enters `reconcileIssue`'s evidence set): `reconcileIssue.authority=CORROBORATED`, `out.issueAuthority=null`, `identityStanding=CONFIRMED`, `marketStanding=EXACT_CURRENT`, `actionAuthority=READY`, `contract.listable=true`, CV/PC exact lookups both allowed — `familyIssueConsensus.outcome='conflicted'` is computed but never reaches the transaction boundary. The fifth false-READY sibling, halt stays active. Narrowest fix traced (feed the runner-up's own non-winning asserted value(s) as conflict evidence — proven to leave Wolverine #90 untouched), not built — awaiting greenlight. `tests/grailkey-directive-aq-canonical-facet-authority.test.js`, 31/31.
