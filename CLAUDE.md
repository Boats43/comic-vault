...
- **Test baseline:** 165 PASS / 16 FAIL / 3 TIMEOUT / 184 total / 0 NEW REGRESSIONS — verified against HEAD `b02353e`, 2026-08-11 (GrailKey Directive H, Item 1), full file-count reconciliation (`ls tests/*.test.js | wc -l` = 184) plus direct re-run of every file this dispatch's changes could plausibly touch. Prior stamp was 164/16/3/183 (HEAD `1c51a8c`, GrailKey Directive G) — this dispatch added exactly one new test file (`grailkey-directive-h-item1-stale-pc-anchor`, 6/6 clean) and grew `grailkey-dispatch-g-task2-pc-anchor-authority` from 7 to 10 assertions (still 0 fail, not a new file), accounting for the entire +1/+1 delta (164→165 PASS, 183→184 total); FAIL and TIMEOUT counts unchanged, list byte-identical to the prior stamp (`decision-engine`, `comp-filter-hygiene`, `sold-verification`, `identity-gate`, `image-search-extraction`, `mega-keys`, `pattern-k-dedupe-issue`, `q-adv397-visual-guard`, `priceBands`, `batch1-fixes`, `grailkey-commit-e/f/g`, `grailkey-commit-v1`, `artist-registry-sync`, `grailkey-dispatch-33-parity-harness` FAIL; `dispatch-42-comicvine-kill`, `grailkey-commit-m-pc-query-fallback` sweep-harness-timeout artifacts + `ship26-integration` genuine intermittent timeout) — none of the 19 unaffected by App.jsx/manualCorrection.js, the only files this dispatch's code changes touched; `tests/q-trackB-commit3-manual-correction.test.js` (466/0, the suite that DOES exercise `manualCorrection.js`) independently re-run clean. Re-sweep methodology note: an automated re-count by exit-code-plus-parsed-assertion-count alone under-counts by exactly one — `grailkey-dispatch-33-parity-harness` deliberately exits non-zero (rc=1) on a "0 passed, 0 failed, 1 skipped" summary, which a naive `FF>0` assertion-failure parse misses entirely; confirmed by direct re-run (see the file's own SKIP-not-PASS docstring). Any future automated sweep must classify by exit code, not by parsing "N failed" out of each file's own summary text. **Standing rule: any dispatch that adds or removes a `tests/*.test.js` file must re-stamp this line in the same commit** — a stamp that silently drifts from the real file count is exactly the "157 vs 159" gap this re-stamp exists to close. This stamp is fresh as of 2026-08-11; treat it as due for re-verification after 2026-08-12 if no closer dispatch has already re-confirmed it.
