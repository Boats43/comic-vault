...
- **Test baseline:** 167 PASS / 16 FAIL / 3 TIMEOUT / 186 total / 0 NEW REGRESSIONS — verified against HEAD (this commit), 2026-08-13 (GrailKey Directive O), full file-count reconciliation (`ls tests/*.test.js | wc -l` = 186). Prior stamp was 166/16/3/185 (HEAD `1d827e7`, GrailKey Directive J, Task 2) — this dispatch added exactly one new test file (`grailkey-directive-o-comp-ladder-reorder`, 5/5 assertions clean, demonstrated failing directly against pre-fix HEAD via git-stash and passing post-fix), accounting for the entire +1/+1 delta (166→167 PASS, 185→186 total); FAIL and TIMEOUT counts unchanged. This dispatch's code change (`api/comps.js` — variant-aware comp-query-ladder reorder, see Pattern Library "GrailKey Directive O") was verified against a 24-file targeted regression set covering every suite that imports `api/comps.js` or otherwise exercises the attempt ladder/filter chain — each file run both pre-fix (git-stashed `api/comps.js`) and post-fix, byte-identical pass/fail output on every one, including the two pre-existing documented-stale suites this change is adjacent to (`comp-filter-hygiene`: 4 failed, both runs; `priceBands`: 7 failed, both runs — unchanged from their existing documented counts, not new regressions). **Standing rule: any dispatch that adds or removes a `tests/*.test.js` file must re-stamp this line in the same commit** — a stamp that silently drifts from the real file count is exactly the "157 vs 159" gap this re-stamp exists to close. This stamp is fresh as of 2026-08-13; treat it as due for re-verification after 2026-08-14 if no closer dispatch has already re-confirmed it.
