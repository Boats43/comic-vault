...
- **Test baseline:** 162 PASS / 16 FAIL / 3 TIMEOUT / 181 total / 0 NEW REGRESSIONS — verified against HEAD `897c9da`, 2026-08-11 (GrailKey Directive E, Task 4), by a full re-run of every `tests/*.test.js` file. The old "157/16/3/177-total" figure was not drift, it was arithmetic — Directive B added two test files (`ship26-integration` fix + 1 other) between when 157 was last stamped and now, and this session added 3 more (`grailkey-dispatch-74-price-coercion`, `grailkey-dispatch-e-task2-asset-confirmation-badge`, `grailkey-dispatch-e-task3-source-labels`), all passing. The 16 FAIL + 3 TIMEOUT file list is byte-identical to the one enumerated in the "Full-suite re-sweep, GrailKey Directive 2026-08-11-C" entry above (`decision-engine`, `comp-filter-hygiene`, `sold-verification`, `identity-gate`, `image-search-extraction`, `mega-keys`, `pattern-k-dedupe-issue`, `q-adv397-visual-guard`, `priceBands`, `batch1-fixes`, `grailkey-commit-e/f/g`, `grailkey-commit-v1`, `artist-registry-sync`, `grailkey-dispatch-33-parity-harness` FAIL; `dispatch-42-comicvine-kill`, `grailkey-commit-m-pc-query-fallback` sweep-harness-timeout artifacts + `ship26-integration` genuine intermittent timeout). **Standing rule: any dispatch that adds or removes a `tests/*.test.js` file must re-stamp this line in the same commit** — a stamp that silently drifts from the real file count is exactly the "157 vs 159" gap this re-stamp exists to close. This stamp is fresh as of 2026-08-11; treat it as due for re-verification after 2026-08-12 if no closer dispatch has already re-confirmed it.
