...
- **Test baseline:** 196 PASS / 19 FAIL / 4 TIMEOUT / 219 total — HEAD (this commit), 2026-08-18 (Directive AU, GK-136/GK-137). Prior 195/19/4/218 (AT). New file `grailkey-directive-au-dellotto-1963.test.js` (24/24) — verified twice, both against a temporarily-reverted pre-instrumentation `reconcileYear`/`reconcileVariantFacet`/`matchCreatorCanonicals` and against the live fix, per the same rigor as PERF-1's baseline diff. No existing test file corrected this dispatch, but one regression was caught and fixed mid-build: the first draft of `YEAR_SOLE_AUTHORITY_PRECEDENCE` (identityReconciler.js) put `'pool-consensus'` in the same non-sole-authority bucket as `'catalog'`, which broke `grailkey-directive-at-year-evidence.test.js`'s own pre-existing "single, uncontested pool-consensus source — CORROBORATED" assertion — corrected to `['user', 'first-eligible-visual', 'pool-consensus']`, re-verified 31/31 on AT's file. FAIL/TIMEOUT byte-identical to prior stamp, diffed programmatically (not eyeballed) against the AT-era baseline — zero new regressions. **Standing rule: re-stamp this line in the same commit as any test-file add/remove.** Full history: Pattern Library "GrailKey Directive AU."
