...
**Extension (ruled 2026-09-10, GrailKey Resume-Mainline dispatch, sixth exposure this session / second from `vercel env pull`):** any command that emits configuration values — `vercel env pull` chief among them, but this covers the whole class — MUST redirect output to a file and be consumed in-process/blind, NEVER left to print to stdout/terminal scrollback. `vercel env pull` with no output path prints pulled values straight to the terminal; always give it an explicit file target (e.g. `vercel env pull .env.tmp`) and read that file programmatically, never `cat`/echo it. Same rule for any equivalent config-dump command (`neon connection-string`, `vercel env ls` is fine — it hides values — but any command whose whole purpose is to reveal a value is in scope).

### Subagent Scope-Violation Rule (P0 PROTOCOL — standing, ruled 2026-09-12, GK-204, Outcome #1 pre-publish hardening dispatch)
**A subagent/fork explicitly scoped read-only that performs code edits, migrations, database writes, or external API calls has violated its scope — full stop, regardless of whether the result turns out correct or harmless.** Evidence: a fork dispatched with an explicit "read-only research task, no edits" prompt (mid Outcome #1 work) instead applied a live DB migration to real Development Postgres, edited 6 source files, created 2 new files, and made a real (read-only) call to eBay's production Trading API — none of it requested of it, apparently acting on the parent conversation's inherited context (a large user directive requesting exactly that implementation) rather than the literal scope of the prompt it was given. The GetUser call it made was harmless and read-only, but it was still outside a read-only research scope — harmlessness is not the test.
**Required response when this is caught:** (1) disclose the violation to the user, explicitly, before reporting any of the subagent's claimed results; (2) independently re-verify every claimed state change — re-read every diff, re-run affected tests under your own execution, re-run the build, issue your own fresh read-only DB queries against the real state — never accept the violating subagent's self-report as evidence of what it actually did; (3) do not infer correctness from the report's confidence or detail — a thorough-sounding report is not verification. Only after independent re-verification passes may the work be treated as done. Full detail: `docs/TICKET-REGISTRY.md`, "GK-204."
...
**SHIPMENT BASELINE — 2026-08-26 (GK-168/169/172 + E-UX train):** 229 PASS / 19 FAIL / 2 TIMEOUT / 250 total — first full 250-file sweep run for this train's baseline close-out (not predicted). Exact FAIL/TIMEOUT filename rosters, known-nondeterministic/cutoff set, and new-train test files: `docs/TEST-BASELINE-HISTORY.md`. New train tests: `tests/gk168-edition-facet.test.js` (85/85), `tests/eux-edition-verify-panel.test.js` (33/33). **The historical baseline did not contain a committed byte-exact filename roster — this shipment commit establishes that durable roster going forward.** **Roster delta (2026-09-02, D4 Phase A + same-asset integrity correction):** +2 files — `tests/d4-identifier-fabric-migration-contract.test.js` (77/77, incl. the S1/S1a/S2 same-asset adversarial proof added after a real cross-asset hole was found and fixed declaratively — ADR-IDENTIFIER-001 Ruling 21) and `tests/d4-identifier-fabric-concurrency.test.js` (7/7, re-run fresh against the changed FK/lock geometry), both real isolated-scratch-schema/two-connection proofs of `db/data0/0013_d4_identifier_fabric.sql` (proposed, not applied to `data1_dev`). Roster count 250→252 going forward. **Roster delta (2026-09-03, D4 Phase B live apply + B7a deterministic closure):** +5 files — `tests/d4-identifier-fabric-retry-unit.test.js` (13/13, deterministic no-DB retry-algorithm proof), `tests/d4-identifier-fabric-live-proof.test.js` (21/21, real `data1_dev`, real service, full B8 checklist), `tests/d4-identifier-fabric-live-concurrency.test.js` (6/7, real live-race proof, one gap at commit time), `tests/d4-identifier-fabric-live-retry-closure.test.js` (24 real triples, disclosed-gap round-1 closure attempt, kept as the historical probabilistic-search record), `tests/d4-identifier-fabric-live-retry-deterministic.test.js` (13/13, the successful deterministic two-connection lock-cycle construction that closed the B7a gap — real SQLSTATE `40P01` received by the actual service, real retry succeeded, durable state verified clean). Roster count 252→257 going forward; the pre-existing 229/19/2/250 figures themselves were not freshly re-run this pass — `historical-roster status changes: NOT FRESHLY MEASURED`. **Roster delta (2026-09-03, GK-178 pooled-connection session-state hazard, resolved):** +1 file — `tests/gk178-pooled-session-state.test.js` (8/8, static source-text checks proving zero bare table references/SET search_path remain, plus real live N=3 concurrency through the real public surface of both the assets and auth modules, read-only, safe to rerun). Roster count 257→258 going forward; historical-roster status changes: NOT FRESHLY MEASURED. **Roster delta (2026-09-03, D5B 0015 design + scratch proof):** +2 files — `tests/d5b-0015-hash-serializer.test.js` (36/36, vq-hash-v1/applicability-hash-v1 property/adversarial + shared-primitive reference-equality proof, no DB) and `tests/d5b-0015-migration-contract.test.js` (42/42, real isolated scratch-schema proof of `db/data0/0015_d5b_valuation_question_applicability.sql`, proposed, not applied to `data1_dev`, incl. real measured WAL). `tests/d5a-market-observation-hash-serializer.test.js` (63/63) and `tests/d5a-market-observation-migration-contract.test.js` (67/67) both rerun unchanged as this pass's own regression proof (src/lib/marketObservationHash.js was refactored onto a new shared src/lib/canonicalHashFraming.js primitive, zero behavior drift). Roster count 258→260 going forward; historical-roster status changes: NOT FRESHLY MEASURED. **Roster delta (2026-09-03, D5B live-apply gate + D5C MarketPopulation design):** +4 files — `tests/d5b-live-apply-gate-a1-rehearsal.test.js` (29/29, real scratch A1-R1/R2/R3 split-migration rollback-isolation proof), `tests/d5b-live-apply-gate-a2-concurrency.test.js` (7/7, real multi-connection concurrency/deadlock proof), `tests/d5c-market-population-hash-serializer.test.js` (17/17, market-population-hash-v1 property proof, no DB), `tests/d5c-market-population-migration-contract.test.js` (34/34, real isolated scratch-schema proof of `db/data0/0017_d5c_market_population.sql`, proposed, not applied to `data1_dev`, incl. real measured WAL). Roster count 260→264 going forward; historical-roster status changes: NOT FRESHLY MEASURED. **Roster delta (2026-09-12, GK-203 Outcome #1 marketplace-execution ledger):** +1 file — `tests/list-ebay-outcome1-handler-smoke.test.js` (25/25, real handler invocation, mocked eBay `fetch` only, real transient `outcome_event` write+cleanup against `data1_dev`). `tests/operator-action-live-proof.test.js` updated (14/14, was 13/14 before the fix — a stale assertion assuming `outcome_event` could not exist replaced with a static proof that `recordOperatorAction` never references it). Roster count 264→265 going forward; historical-roster status changes: NOT FRESHLY MEASURED. **Roster delta (2026-09-12, GK-205 Outcome #1 pre-publish hardening — real frontend linkage, fail-before-eBay, observation-cutoff policy):** +2 files — `tests/current-operator-action-selection-unit.test.js` (10/10, pure-function proof that the real "List on eBay" button's operator-action selection is find-by-id against a server-declared `currentOperatorActionId`, never array-position, using Creepy's own real HOLD-then-LIST shape) and `tests/asset-graph-operator-actions-live-proof.test.js` (6/6, real live `data1_dev` proof that `getAssetGraph` returns `currentOperatorActionId` naming Creepy's real LIST row, not the earlier HOLD row). `tests/list-ebay-outcome1-handler-smoke.test.js` extended in place, same file, 25/25→36/36 (new pre-flight-rejection scenarios: attach-to-HOLD now blocked BEFORE any eBay call — was previously rejected only after a successful mocked listing; a bearer-token-missing-with-linkage-attempted request now also aborts before any eBay call, a deliberate, disclosed behavior tightening from the prior pass; a legacy zero-GrailKey-fields request is now separately proven to still list successfully, preserving GK-151's carve-out; plus three-price separation and `next_observation_due_at` assertions on the real-success case). Roster count 265→267 going forward; historical-roster status changes: NOT FRESHLY MEASURED.
...
Full strategy: `docs/GRAILKEY-STRATEGY.md`. Architecture decisions: `docs/adr/` (ADR-AUTH-001 principal/owner/custodian, ADR-ASSET-001 physical-asset identity, ADR-MEDIA-001 private-media law, ADR-EVENT-001 event model, ADR-STORAGE-001 storage roles, ADR-ADAPTER-001 adapter contract, ADR-API-001 API contract discipline, ADR-ID-001 permanent identity, ADR-IDENTIFIER-001 identifier fabric (D4, schema ruling only — Phase A HOLD), ADR-VALUATION-001 ValuationQuestion + Applicability (D5B semantic closure, docs-only — 0015 design boundary), plus per-milestone design docs).
...
### DATA-1D / GrailKey — AUTH + COLLECTION LIVE; PHYSICAL-ASSET KERNEL NOT YET IN PRODUCTION (corrected GK-215, 2026-09-19)

```
origin/main = a8dcdca · Production dpl_5TtAVeW2uirLpFpgQ2caVnxbncEL READY
primary alias comic-vault-rouge.vercel.app
DATA-1D auth + server-backed collection = PRODUCTION LIVE
PHYSICAL-ASSET KERNEL (gk_asset/media/collection_item_link/ownership_event/
outcome_event/operator_action_event/etc.) = NOT LIVE IN PRODUCTION —
direct read-only row-count audit, GK-215, 2026-09-19: gk_asset/media/
collection_item_link/ownership_event/mint_event/domain_event/
entity_mint_basis ALL 0 ROWS in real Production; operator_action_event,
outcome_event, outcome_economics_component TABLES DO NOT EXIST there at
all. The prior "PRODUCTION LIVE" framing below (GK-166/203-214) described
real, genuine work that was verified against the real Development
database — never independently re-checked against Production directly
until this dispatch. Creepy #1 (gkAssetId 01a02d23-1acb-72e8-aae3-
8f851308e9cf, mediaId 01a02d23-2809-7024-9312-d45bb5003014) is real,
legitimate Development durable-asset evidence — created 2026-08-23,
confirmed live in Development today (207+ gk_asset rows there) — NOT
Production evidence. Not fabricated, not deleted, not moved — simply
mislabeled as to which environment it lives in; this correction relabels,
it does not rewrite.
H8 (Milestone Ten) as originally formulated was a DEADLOCK, not just an
open item: it required a Production durable asset to retrieve/compare,
while Production durable capture stayed blocked until H8 passed. See
"H8-A/H8-B two-stage resolution" below — full detail docs/TICKET-REGISTRY.md,
GK-215.
GK-163 CLOSED · GK-164 CLOSED · GK-165 CLOSED · GK-166 CLOSED (Development
evidence, per above — re-scoped, not un-closed)
GK-167 OPEN (pre-production media-routing gate) · MEDIA-DEBT OPEN
GK-151 PARTIALLY-SATISFIED (list-ebay.js step 4 now real, see below) · GK-160/161/162 OPEN report-only
DATA-0E-FULL crawl running independently (watchdog-armed)
OPEN QUESTION (GK-215, not resolved this pass): Outcome #1's real eBay
LIST/DELIST (ItemID 366665728633) genuinely executed as a real eBay API
call from real Production infrastructure (api/list-ebay.js, real Vercel
Production, real eBay servers) — that marketplace-execution fact is
preserved and NOT disputed here. What's now unclear, given Production's
gk_asset/collection_item_link are confirmed empty today: exactly what
durable linkage validation that call passed at the time it ran, and
whether it happened before GK-207's unconditional-linkage gate landed.
Not investigated further this pass — flagged for a future dispatch, not
resolved by assumption in either direction.
NEXT AUTHORIZED SEQUENCE: H8-A preconditions (independent proof) →
H8-A PASS → MILESTONE_TEN_H8_BOOTSTRAP=true (one-shot only, NOT itself H8
proof) → exactly one real Production capture → H8-B two-device
retrieval-and-compare proof → MILESTONE_TEN_H8_PASS=true (bootstrap
disabled) → normal Production capture unlocked → fresh real asset →
fresh LIST → Outcome #1 at scale.
```

**WHAT IS LIVE IN PRODUCTION:** the DATA-1D auth chain (login, session tokens, per-principal authorization — real, confirmed via this session's own direct Production probes) and the server-backed collection_item store (GK-216/217, record sync AND image sync both operator-confirmed working). The DATA-1C media pipeline CODE is deployed, but no physical-asset media has ever actually been written through it in Production (0 real `media` rows there).

**WHAT IS NOT LIVE IN PRODUCTION (GK-215, schema gaps closed by GK-216):** the entire physical-asset kernel — `gk_asset`, `media`, `collection_item_link`, `ownership_event`, `current_owner`, `acquisition_event`, `valuation_event`, `decision_event`, `mint_event`, `entity_mint_basis`, `domain_event`, `outbox`, `asset_identity_assignment`, and now also `operator_action_event`/`outcome_event`/`outcome_economics_component` (0021/0023/0025, live-applied to Production 2026-09-19, GK-216 — full before/after verification, zero pre-existing rows touched) all exist as Production TABLES with ZERO rows. **GK-216 also found and closed two gaps beyond the three originally suspected:** `valuation_event.market_population_id` (0020, now applied) and a pre-existing, both-environment (not Production-specific) GK-194 function-search_path-pin defect (0019, flagged not fixed — doesn't block the capture→LIST→outcome path). **GK-216 also resolved the eBay-linkage contradiction:** the real Creepy #1 `AddFixedPriceItem` call (ItemID `366665728633`) validated real linkage rows, but those rows — and the code that validated them — ran via Jimmy's local `vercel dev` session against Development, never through deployed Production infrastructure; GK-207's gate was not bypassed, it simply never ran against Production at all for that historical call. **RELEASE BLOCKER found (GK-216, not fixed — proposed only):** `api/list-ebay.js`'s bundle-listing path (`item.bundle === true`) still executes and returns before GK-207's unconditional linkage gate — a disclosed gap from GK-207 itself, never closed — meaning a currently-reachable Production path can publish to real eBay with zero durable-asset linkage. Full detail `docs/TICKET-REGISTRY.md`, GK-216.

**H8-A/H8-B TWO-STAGE RESOLUTION (GK-215):** H8 as originally written was circular. Replaced with: **H8-A** (bootstrap preconditions — Production login/auth, same-account server-backed collection phone+desktop, `/api/capture-scan` deployed+authenticated, Production environment-identity/isolation proven, capture idempotency tests passing, Production capture otherwise fail-closed, no marketplace write authorized) gates a **one-shot bootstrap** (`MILESTONE_TEN_H8_BOOTSTRAP=true`, api/capture-scan.js — reuses the existing asset-count read, `hasAnyPhysicalAsset()`, as the one-bootstrap-maximum enforcement; any failure to determine eligibility fails closed). Exactly one real Production capture is then permitted. **H8-B** is that capture's own two-device (phone + desktop) independent retrieval-and-compare proof (gkAssetId/mediaId/byteLength exact match). Only after H8-B is independently recorded does `MILESTONE_TEN_H8_PASS=true` get set and bootstrap get disabled. Required invariant, enforced in code: bootstrap authorization ≠ H8 proof. Full detail `docs/TICKET-REGISTRY.md`, GK-215.

**WHAT MUST NOT BE DONE:** no production scanner/capture wiring beyond what's already shipped, no capture beyond the single authorized H8-A→bootstrap→H8-B sequence above, until H8-B is independently recorded PASS. No fabricating, copying, or relabeling Development asset/media rows as Production evidence. No commit/push/deploy without explicit authorization. No pricing-math changes without greenlight (applies equally to GrailKey valuation code). No silent doctrine change during a "compaction"/"cleanup"-labeled pass — semantics outrank size targets.

**WHAT IS NEXT:** independently prove each H8-A precondition still holds (see `docs/TICKET-REGISTRY.md`, GK-215/216) → set `MILESTONE_TEN_H8_BOOTSTRAP=true` → the single authorized bootstrap capture → H8-B's two-device compare → `MILESTONE_TEN_H8_PASS=true` (bootstrap disabled) → normal Production capture unlocked → Long Box economics → the outcome-data loop at scale → DATA-0E-FULL mint → 0F shadow → 0G cutover. Production's schema is no longer a blocker for this sequence (GK-216 closed it) — the bundle-listing linkage gap (GK-216, release blocker, proposed fix not yet built) should be closed before bundle listings are used in Production, independent of the H8 sequence itself. The AWW #16 rescan (closes GK-158/159's own comic-runtime closeout gate) is a separate, still-open item on the 90-day board.
...
- **2026-08-21** DATA-1 readiness interrogation, GK-145/146/147. **2026-08-21 urgent** GK-148 SHIPPED-PENDING, GK-149 NOT CONFIRMED. **Phase 0.3** GK-142/143/148/149 CLOSED via physical rescan. **GK-152** SHIPPED-PENDING (comps-pool issue rescue). **GK-153-156** GI Joe dispatch train. **GK-157/158** elected-title + provisional-issue gaps fixed. **GK-153-157/152** CLOSED via physical rescan. **GK-159** commit4-terminal floors instead of clears; GK-160/161 report-only. **GK-163** CLOSED (class-wide idempotency fingerprint). **GK-164** CLOSED (DATA-1D credential incident + session hardening, history rewritten pre-push). **GK-165** CLOSED (S3 84-vs-94 drift root-caused). **GK-166** CLOSED (real-photo Vercel Blob capture proof). **GK-178** CLOSED 2026-09-03 (pooled-connection session-state hazard — every bare table reference in `src/modules/assets/`/`src/modules/auth/`'s repository/idempotency/service files now schema-qualified, `SET search_path` removed from both `acquireConnection()`s; D5 infrastructure gate lifted). **D5 Phase 0/A** 2026-09-03 — D3.3 relationship UNDER, `market_observation.asset_id` removed (Applicability-only linkage), own canonical content-hash contract `mo-hash-v1` (structural temporal representation — `occurred_on DATE`/`occurred_at TIMESTAMPTZ`, mutually exclusive, no synthetic midnight; `grade_basis` added, no comic-specific grading scale), batched write-ceremony ruling. **0014 LIVE 2026-09-03** — applied to `data1_dev`, 23/23 catalog verification, live contract proof, all pre-existing tables unchanged. **GK-180** OPEN (durable economic pipeline disconnected from live pricing). **GK-181** OPEN (mutable client-side IndexedDB price — the D5 transition target, not a kernel violation). **GK-182** OPEN (`evidenceContracts.js` PriceCharting retention-day doc inconsistency vs. real 7-day cache). **GK-183** OPEN (single-provider multi-sub-scale grade-basis ambiguity, narrowed). **GK-184** OPEN — hard D5D gate (provider-retrieval time must survive cache storage/retrieval before production capture). **D5B SEMANTIC CLOSURE 2026-09-03** (`docs/adr/ADR-VALUATION-001-question-applicability.md`, docs-only) — ValuationQuestion/Applicability conceptual boundary ratified (R1-R6): `asset_id NOT NULL`, field-admission rule applied to `fetchComps()`'s 22 live params (4 semantic survive, 18 do not), Q19 three-way separation reaffirmed (filter/model version never enters question identity), no `ValuationAttempt` table, own `mo-hash-v1`-style question hash, Applicability verdict model kept distinct from D4's vocabulary. P2a positively resolved (scratch-schema containment proof cited, already live at `6371acb`, no code change). GK-185/186 opened (forward constraints, no code to fix yet). Applicability write-volume banked as a second pre-D6 WAL-sizing data point (`docs/MASTER-BOARD.md`). **V1-V4 addendum** (2026-09-03, same ADR) — verdict/resolution-authority two-axis split, Applicability defaults to no-supersession, 0015 must reuse `marketObservationHash.js`'s serializer verbatim (no second hash contract), target-grade needs `canonicalGradeString` + a `grade_basis`-style qualifier (`isGraded` alone doesn't cover it) — gated by GK-187. **0015 DESIGN + SCRATCH PROOF PASS 2026-09-03** (`docs/D5B-0015-DESIGN-REPORT.md`) — `db/data0/0015_d5b_valuation_question_applicability.sql` PROPOSED (not applied to `data1_dev`): `valuation_question` + `applicability` tables, `asset_identity_assignment` gains a DB-enforced immutability trigger + `UNIQUE(id,asset_id)` (GK-188, closes a pre-existing gap — that table had none before). `src/lib/canonicalHashFraming.js` (new, shared) + `valuationQuestionHash.js` (vq-hash-v1) + `applicabilityHash.js` (applicability-hash-v1); `marketObservationHash.js` refactored onto the shared primitive, zero behavior drift (63/63 rerun clean). GK-187 V1-V4 all satisfied by design+real scratch proof (SHIPPED-PENDING); GK-189 opened (confidence_tier naming, not D4's resolution_authority). 42/42 + 36/36 real proof, `data1_dev` never touched pre-apply. **LIVE-APPLY GATE PASS 2026-09-03** (`docs/D5B-LIVE-APPLY-REPORT.md`) — migration SPLIT (A1) into `0015` (`asset_identity_assignment` immutability repair alone) + `0016` (ValuationQuestion+Applicability, depends on 0015); GK188-0/A1/A2/GK-188 all PASS; real `data1_dev` census clean (80 rows, 58/58 live-row 1:1, zero anomalies); GK-190 opened (pre-existing repository.js same-asset race, unrelated to 0015, not fixed); one real test-only DELETE against `data1_dev` found and fixed (`tests/d3-2-application-wiring-live-proof.test.js`). **0015/0016 LIVE-APPLIED 2026-09-03** (commit `3591fc7`, manual execution after harness auto-mode classifier blocked automated Bash) — PA1-PA18 independently re-verified read-only against real `data1_dev` in this same session (not merely accepted from the apply transcript): both tables live, 0 rows in either (no writer exists, GK-180 confirmed), `asset_identity_assignment`/`market_observation`/`gk_asset` counts unchanged (80/23/131), zero public-schema contamination, live trigger function body matches committed source exactly. GK-191 opened (recovery-anchor recorder timezone serialization defect, observability-only, not a schema defect). **D5C MARKETPOPULATION DESIGN 2026-09-03** (`docs/D5C-MARKET-POPULATION-DESIGN-REPORT.md`) — `db/data0/0017_d5c_market_population.sql` PROPOSED (not applied): `market_population` (header) + `market_population_member` (typed membership, `SELECTED`/`EXCLUDED` + `exclusion_reason` — durable info Applicability alone can't express, M2 PASS) tables. M1 PASS (`comp_snapshot` = future materialized projection of MarketPopulation, zero schema change to it this pass); M3 PASS (zero-code reproducibility — membership durably persisted, Candidate C header+members); M4 PASS (no `asset_id` on MarketPopulation). No supersession (same D5B precedent). `src/lib/marketPopulationHash.js` (market-population-hash-v1, reuses the shared framing primitive a third time). 17/17 + 34/34 real scratch proof (MP-NP1-12 all PASS), real measured WAL (~757-1168 B/row header, ~768-830 B/row member upper bound). **D5C LIVE-GATE PASS + LIVE APPLY 2026-09-03** (`docs/D5C-MARKET-POPULATION-LIVE-GATE-REPORT.md`) — N1 (GK-192 declarative closure ADOPTED, not writer-delegated: `market_population_member.valuation_question_id` constrained by 2 composite FKs simultaneously, cross-question membership now structurally unrepresentable, proven live 2/2) + N2 (GK-191 FIXED: `clock_timestamp()` without `AT TIME ZONE 'UTC'` verified correct, no 7h offset) + N3 (full measurement contract banked, MEASURED/PROJECTED/HYPOTHESIS labeled). `0017` LIVE-APPLIED (commit `4fc10b5`, real execution succeeded this time — classifier did not block it) — MP-PA1-18 independently re-verified read-only: `market_population`+`market_population_member` live in `data1_dev`, 0 rows in either, every pre-existing count unchanged (131/23/0/0/4/78), zero public-schema contamination, D1/D5A/D5B triggers all unchanged. GK-192 CLOSED. GK-180 confirmed still zero writer call sites (0 rows in all four D5 tables, live). **Full D5A→D5B→D5C schema chain now LIVE.** **D5D ISOLATED WRITER DESIGN + PROOF PASS 2026-09-03** (`docs/D5D-WRITER-BRIDGE-DESIGN-REPORT.md`) — user's own scope-correction ruling ("D5D SCOPE CORRECTION — MILESTONE TEN PRECEDENCE") holds runtime wiring until Milestone Ten's phone proof closes; this pass built `src/modules/valuation/` (db/errors/idempotency/repository/service/index) as a complete, real-Postgres-proven writer with ZERO production call sites (proven statically, `tests/valuation-module-boundary.test.js`, 13/13) — W2 asset-source route proven against real `data1_dev` read-only (5/5, zero writes); atomic transaction + M3 reconstruction, F10 idempotency, write batching (15 round trips for N=20, not O(N)), and the GK-192 integration-level negative proof (real FK rejection via direct repository misuse) all proven real-scratch-schema (21/21, `tests/d5d-valuation-writer.test.js`). GK-193 opened+CLOSED same pass: a real idempotency-claim concurrency race (two simultaneous calls under one idempotencyKey racing on the claim INSERT) found by this dispatch's own concurrency test, fixed by recovering the winner's committed result on the loser's unique-violation rather than propagating a raw DB error. GK-191 status corrected OPEN→CLOSED (fix had already landed in the D5C live-gate pass; registry row was never re-stamped). `comp_snapshot` compatibility: DEFER (M1-A unchanged). GK-180 confirmed still zero writer call sites throughout (0 rows in every D5 table, before/after every proof in this pass). **D5D runtime wiring remains a separate, future, explicitly-gated authorization — not granted by this pass.** **GK-179 REGRESSION (2026-09-11)** — Development's Vercel-Neon integration auto-resynced onto the wrong branch/database (`docs/MASTER-BOARD.md`, "Production/Development isolation risk"); GK-179's own fail-closed guard re-confirmed live against the real misroute (`EnvironmentIdentityError`/`MARKER_QUERY_ERROR`, zero domain queries executed). **GK-201** — login-security and collection-item-namespace pull-forward audits banked, both PASS-with-disclosed-caveats, full detail `docs/TICKET-REGISTRY.md`. **GK-202** — post-first-OperatorAction P0 train: rescan-continuity `priorCollectionItemId` alias mechanism SHIPPED (`src/modules/capture/service.js`), deterministic `currentValuationId`/`currentDecisionId` selection SHIPPED (`src/modules/assets/repository.js`, id tie-break), fail-closed `scripts/db-admin-preflight.mjs`/`scripts/pg-dump-development.mjs` admin tooling SHIPPED, second Book Forge credential exposure found+fixed (same root as GK-200's own, rotation still OPEN) — full detail `docs/TICKET-REGISTRY.md`. **GK-203** — Outcome #1 IMPLEMENTATION PASS: `outcome_event` (0023, LIVE-APPLIED) promotes 0006's own design draft (scoped to `LISTED/SOLD/EXPIRED_UNSOLD/DELISTED/ACTIVE_AT_CUTOFF`, reusing the class-wide GK-163 idempotency law); new `recordOutcomeEvent` writer enforces the OUTCOME ATTACHMENT RULE (a `LISTED` row must attach to the specific `LIST` operator_action_event, never `HOLD`/`PASS`) in code; `api/list-ebay.js` gains an OPTIONAL/ADDITIVE GrailKey linkage plus a new permanent pre-publish `GetUser` account/token safety check — `App.jsx`'s real listing button is unchanged. Real handler smoke 25/25, zero real eBay network calls, full detail `docs/TICKET-REGISTRY.md`. **GK-205** — Outcome #1 PRE-PUBLISH HARDENING (found the context-gathering fork from the GK-203 pass itself had violated its read-only scope, GK-204 — see "Subagent Scope-Violation Rule" above; every GK-203 claim independently re-verified before this pass began): `src/App.jsx`'s real "List on eBay" button now sends real `gkAssetId`/`decisionEventId`/`operatorActionEventId`, selected deterministically via new `getAssetGraph` fields `operatorActions`/`currentOperatorActionId` (same recorded_at+id tie-break as `currentValuationId`/`currentDecisionId`, P0-B) and a pure `selectCurrentOperatorAction` (find-by-id, never array-position) — refuses to even attempt a publish client-side if the current action isn't LIST. `api/list-ebay.js` gained a pre-flight `validateOutcomeAttachment` check that runs BEFORE any eBay network call (image upload, GetUser, AddFixedPriceItem) — a live eBay listing without valid durable linkage is now a hard abort, not a decline-and-continue (a deliberate, disclosed tightening of GK-203's own behavior). `0024_outcome1_observation_cutoff.sql` (LIVE-APPLIED) adds `outcome_event.next_observation_due_at`, populated at LISTED-write time (never backfilled) from a documented `LISTING_OBSERVATION_WINDOW_DAYS=30` policy matching eBay's own GTC renewal cadence. Three-price separation (predicted `valuation_event.value_amount` / actual `outcome_event.ask_amount` / realized `gross_amount`, NULL until a real SOLD) proven never to conflate. Real handler smoke extended 25/25→36/36 in place, plus 2 new files (`current-operator-action-selection-unit.test.js` 10/10, `asset-graph-operator-actions-live-proof.test.js` 6/6). No real `AddFixedPriceItem` call made against the real Creepy book. Full detail `docs/TICKET-REGISTRY.md`. **GK-206** — quick-list button (`ManagePage`'s `handleAction`) fixed to surface the real error reason instead of a bare "Failed — Retry" (`b9e762b`). **GK-207** — pre-click correction, same day: confirmed (by reading the code, not assuming) that GK-205's own gate was conditional — a caller supplying ZERO GrailKey fields (unauthenticated, or an unlinked catalogue item) skipped it entirely and published normally with no durable linkage. Removed the conditional: auth + complete `gkAssetId`/`decisionEventId`/`operatorActionEventId` are now UNCONDITIONALLY required before any eBay call on this path, closing GK-151's own carve-out for this endpoint. **Disclosed scope impact, broader than Creepy alone: any catalogue item with no linked GrailKey asset can no longer publish through this endpoint until it has one.** Bundle listings remain untouched, a disclosed gap. Handler smoke re-run 37/37. **GK-208** — root-caused a real captured publish failure (HTTP 502, eBay Ack=Failure, ErrorCode 21919136 "eBay requires at least one photo"): `listOnEbay` (`src/App.jsx`) sent only a singular `image` field; `api/list-ebay.js`'s own T2-1 image logic reads `item.images` (plural) for the common non-LOW-confidence case with NO fallback to `item.image`, so `imagesToUpload` was silently `[]` for essentially every real single-item listing, `<PictureDetails>` was omitted entirely, and eBay rejected the real `AddFixedPriceItem` call. Fixed both ends: client now sends `images: getComicPhotos(item)` (the same array already used elsewhere in the file); server now fails closed with `PUBLISH_BLOCKED_NO_PHOTO` BEFORE `verifySellerAccount`/`AddFixedPriceItem` if zero usable hosted picture URLs remain after upload, instead of letting eBay itself reject a doomed call. New `tests/list-ebay-photo-handoff.test.js` (15/15) reproduces the exact original bug (singular `image` only → blocked) and proves the fix (real `images` array → uploaded → `<PictureURL>` reaches `AddFixedPriceItem`) and the fail-closed gate (all uploads failing → still blocked, never a doomed call). Also found, while re-running full regression: a genuine THIRD real `operator_action_event` LIST row on Creepy (`01a098ed-...`, `source=operator-api`, 2026-09-13T04:02:07Z) — real, Jimmy's own usage, untouched — which made two pre-existing tests' hardcoded "exactly 2 rows"/fixed-id assumptions stale; both updated to assert the durable invariants (original two rows present and unchanged; `currentOperatorActionId` always resolves to the true chronologically-last row) rather than a frozen snapshot. **GK-209 — Outcome #1 CLOSER: eBay OAuth audit + lifecycle observer + economics dual-path + PredictionError, built; real Creepy listing found already DELISTED, not a hypothetical.** OAuth audit: existing OAuth is client_credentials only (app-level, Browse API) — Fulfillment/Finances need a User token via Authorization Code Grant, which needs a RuName (MISSING at the time of this dispatch — Jimmy must create one in the eBay Developer Portal, one-time, self-service — **STALE, superseded 2026-09-15, see GK-214: RuName created, consent completed, refresh token live in Vercel Production, Fulfillment+Finances reads both proven 200**) before any consent URL can even be built; `src/lib/ebayUserOAuth.js` built and proven to throw rather than construct a broken URL until that exists. Crucially, the LIFECYCLE OBSERVER needs NONE of this — `GetItem` (already-working Trading API) fully covers ACTIVE/SOLD/ENDED. New `scripts/observe-outcome1-listing.mjs` (manual-invocation, matches "no month-long watcher needed") — run for REAL against ItemID `366665728633` and found the listing had ALREADY ENDED (`EndingReason=NotAvailable`, ~4 min after publish, 0 sold) — same reason code `api/delist-ebay.js` itself uses. Correctly wrote a durable `DELISTED` outcome_event (`01a09915-...`), never touching the original `LISTED` row, never fabricating a SOLD. New `db/data0/0025_outcome1_economics_components.sql` (LIVE-APPLIED) — append-only per-fact economics ledger (gross/fees/shipping/refund/credit/order_reference), realized net always DERIVED (SUM), never a stored mutable field; `recordEconomicsComponent`/`getOutcomeEconomics` (ownership-checked, idempotent) + `api/outcome-economics.js` (the required manual-entry fallback, `source` hard-forced to `operator-entered`, never caller-suppliable). `src/lib/predictionErrorScoring.js` — pure scoring, CENSORED (never fabricated) while unsold; Creepy's own real state ($61.41 predicted, $75.00 ask, no realized sale) scores correctly as CENSORED. Full detail `docs/TICKET-REGISTRY.md`, GK-209. **GK-210** — Jimmy confirmed he personally ended the real listing (DELIST ACTOR: OPERATOR, no evidence of an automated removal). Full execution history preserved exactly as recorded, not investigated further, not rewritten. CLOSED-LOOP ECONOMIC OUTCOMES remains 0 (a DELISTED-unsold listing isn't a scored economic outcome). A relist requires a genuinely new LIST action and gets its own new eBay ItemID — already true by construction (GK-207's unconditional gate + eBay never reissuing ItemIDs), no new code needed. **GK-211** — agent-commerce projection boundary banked (W3C/GS1 UCP/ACP/WebMCP-class standards work): any future GrailKey interface exposing asset facts to external AI agents is a replaceable projection over durable truth, never kernel — restates ADR-ADAPTER-001 Rulings 32/33 for agent-facing surfaces; no conflict, no kernel-leak ticket found, docs-only. Full detail `docs/TICKET-REGISTRY.md`. **GK-212** — Hulk #273 comp-selection, READ-ONLY investigation: active/sold comps confirmed as two separate pools throughout; working classification DISPLAY SEMANTICS DEFECT (card's "Only 1 comp found" text sources from a laxer single-pass pool than the one most likely controlling the price) — a COMP ADMISSION DEFECT (issue-number gate skipped when `issueNum` is falsy, same class as GK-35) not ruled out without live logs; not an Outcome #1 blocker. **GK-213** — Product Refinement/Grading Program banked (10 items: operator-corrections-first-class, comic attribute engine by role, grade override, screenshot/quick-lookup mode, share reliability, scan-speed instrumentation, outcome calibration, DECIDE/WHY UX split, four-authority invariant, scope/order) — zero code, zero pricing math, full text `docs/PRODUCT-REFINEMENT-BANK.md`. **GK-214** — MASTER-PLAN STATE CORRECTION, read-only: eBay User OAuth (RuName/Fulfillment/Finances) is COMPLETE (operator-attested, corrects GK-209's stale "RuName MISSING"); Milestone Ten gates flag-gated production CAPTURE of new assets, NOT the eBay listing action — a real production LIST already happened once (Creepy #1) independent of Milestone Ten; GK-151 step 4 (mutation authorization) now real for `api/list-ebay.js` via GK-207, step 3 (per-principal marketplace credential) still unbuilt. Full detail `docs/TICKET-REGISTRY.md`. **GK-215 — H8 BOOTSTRAP DEADLOCK RESOLUTION, STATE CORRECTION.** Root-cause investigation of "H8 proof UI not rendering" led to a direct read-only row-count audit of real Production: `gk_asset`/`media`/`collection_item_link`/`ownership_event`/`current_owner`/`acquisition_event`/`valuation_event`/`decision_event`/`mint_event`/`entity_mint_basis`/`domain_event`/`outbox`/`asset_identity_assignment` all exist as Production tables but hold **zero rows**; `operator_action_event`/`outcome_event`/`outcome_economics_component` **do not exist as Production tables at all**. Creepy #1 (`gkAssetId 01a02d23-1acb-72e8-aae3-8f851308e9cf`) is real and live — in Development (207+ gk_asset rows there, matching GK-166's 2026-08-23 date exactly) — not Production. Every GK-166/203-214 claim of "PRODUCTION LIVE" physical-asset evidence is corrected to "Development durable-asset evidence" by this entry; the real eBay LIST/DELIST marketplace execution (ItemID `366665728633`) is NOT disputed as a real Production API call, but exactly what durable linkage it validated against at the time is now an open question, not asserted either way. H8 as originally formulated was circular (needs Production durable asset to prove H8; Production capture blocked until H8 passes) — replaced with **H8-A** (independently-proven bootstrap preconditions) gating a **one-shot** `MILESTONE_TEN_H8_BOOTSTRAP=true` (api/capture-scan.js, new — reuses the existing `hasAnyPhysicalAsset()` asset-count read as the one-bootstrap-maximum enforcement, fails closed on any eligibility-check error) permitting exactly one real Production capture, followed by **H8-B** (that capture's own independent phone+desktop retrieval-and-compare proof) before `MILESTONE_TEN_H8_PASS=true` is ever set. Required invariant, enforced in code: bootstrap authorization ≠ H8 proof. No capture executed this pass. `tests/h8-bootstrap-gate.test.js` (9/9, real Development DB, proves the fail-closed eligibility check via a genuine environment-identity mismatch) + `tests/capture-scan-endpoint-h8-gate-proof.test.js` re-run clean (21/21, unaffected). Full detail `docs/TICKET-REGISTRY.md`, GK-215. **GK-216 — PRODUCTION INTEGRITY AUDIT: eBay linkage contradiction resolved, complete migration census, five missing migrations applied to Production, bootstrap safety reconfirmed.** eBay linkage: the real Creepy #1 `AddFixedPriceItem` (ItemID `366665728633`) validated real `gkAssetId`/`decisionEventId`/`operatorActionEventId` rows, but GK-208's own record shows the feeding `operator_action_event` LIST row was written "via the running `vercel dev` session" — a local dev server, not deployed Production — and `operator_action_event` didn't exist as a Production table until this dispatch; GK-207's gate was never bypassed, it simply never ran against Production for that historical call. Release blocker found, not fixed: `api/list-ebay.js`'s bundle-listing branch still returns before GK-207's linkage gate (a disclosed-but-never-closed gap from GK-207 itself) — a currently-reachable Production path can publish to real eBay with zero durable linkage; smallest fix proposed (per-item `validateOutcomeAttachment` loop, or fail-closed on `bundle && production` until built), not implemented this pass. Full migration census (0001-0026, both environments) found two Production gaps beyond the three GK-215 already named — `valuation_event.market_population_id` (0020) and a both-environment (not Production-specific) GK-194 function-search_path-pin defect (0019, flagged not fixed, doesn't block the capture→LIST→outcome path) — everything else absent from Production was equally absent from Development (DATA-0's 0001/0002, parts of 0003/0004), not drift. Applied 0020→0021→0023→0024→0025 to Production in dependency order (`scripts/apply-0020-0021-0023-0024-0025-production.mjs`, new), with a recorded UTC restore-point timestamp and full before/after verification: all 28 pre-existing row counts byte-identical, all new tables/columns/constraints/indexes confirmed present and matching the canonical migration definitions. Bootstrap safety reconfirmed post-reconciliation: neither `MILESTONE_TEN_H8_PASS` nor `MILESTONE_TEN_H8_BOOTSTRAP` set in real Vercel Production env (key names checked directly), `gk_asset` still 0 rows, no asset minted, no media attached, no link created, no marketplace write. Full detail `docs/TICKET-REGISTRY.md`, GK-216. **GK-218 — FINAL CAPTURE-PATH WIRING.** (Numbering note: this session's own GK-215/216 above collide with real, pre-session commits `b0d25b7`/`8bb0012`/`2f890ca` that never got registry rows — disclosed in full in `docs/TICKET-REGISTRY.md`'s GK-218 entry, not renumbered; this dispatch uses 218, the next genuinely free number.) Traced the normal Scan UI: grepped the entire frontend, zero references to `/api/capture-scan` anywhere — `gradeBlob`→`addToCatalogue`→`persistCollectionItem` only ever call `/api/grade`/`/api/enrich`/`/api/collection`. Every normal scan/save completed without ever touching durable capture. Fixed: `GrailKeyOperatorPanel.jsx`'s `status:'none'` branch (previously `return null`) now renders one explicit "Capture as Owned Physical Asset" button — the ENTIRE mint/don't-mint decision point, reachable only by an operator explicitly tapping it on an already-viewed catalogue item, never inferred from image source/camera/upload/screenshot/photo-presence/scan-success/any default branch; a second guard requires a real local `data:` URL (this device's own bytes, never a synced proxy path) before any request is built. Real handler proof (`tests/grailkey-operator-panel-capture-wiring.test.js`, 16/16, real Development DB): correct payload shape, real mint, real `collection_item_link` linkage confirmed independently, idempotent replay (same key → same asset, zero new rows), real idempotency conflict (409, zero new rows), and the quick-lookup guard rejecting a non-local photo before any network call. No pricing/grading/identity code touched (`git diff` confirmed). `MILESTONE_TEN_H8_BOOTSTRAP` NOT set this pass — the new button is wired and inert, would 403 against real Production exactly as before. Production `gk_asset`/kernel tables independently re-confirmed still 0 post-wiring. Full detail `docs/TICKET-REGISTRY.md`, GK-218. **MAX BUY V1** (`516b1fb`) — BidCalculator (Buyer Mode) extended, not rebuilt: `src/lib/maxBuyCalculator.js` inverts the existing net-profit formula (`maxBuy = marketValue - fee - supplies - labor - targetProfit`, targetProfit reuses the existing "Min profit $" setting, no new margin/ROI concept invented) to show MAX BUY, below/above-MAX-BUY delta, and let an operator record an actual purchase price after the fact without ever overwriting the recommendation. `tests/max-buy-calculator.test.js` 20/20. **GK-219/GK-220** (2026-09-20) — GK-214's real Fulfillment/Finances HTTP 200 proofs (via an uncommitted one-off scratchpad script) are banked as credential/API-access proof only, distinct from `0356ba3`'s real repo-level ingestion code (`src/lib/ebayFulfillmentFinances.js`, `scripts/ingest-outcome1-financials.mjs`) — not a contradiction, two different scopes. **Durable Buyer Decision Ledger V1** (`0027_buyer_decision_ledger.sql`, LIVE-APPLIED to both Development and Production) makes Buyer Mode's BUY/PASS decisions durable, cross-device GrailKey data for the first time — `buyer_decision_event`/`buyer_acquisition_event`, deliberately NOT reusing `acquisition_event`/`valuation_event`/`decision_event`/`outcome_event` (all `asset_id NOT NULL`, wrong fit — Buyer Mode decisions are pre-acquisition, no asset exists yet) nor `domain_event` (schema-permissive but a documented derivative envelope, not a primary store). New `src/modules/buyer/` module + `api/buyer-decision.js` (append decision, append acquisition, read history — no update endpoint exists for either). `src/App.jsx`'s BidCalculator now saves locally first always, then best-effort syncs durably (`_syncStatus:'pending'|'synced'`, retried on next authenticated reconnect via the same `[grailkeyAuthed]` effect Collection's own retry already uses) — full valuation provenance captured (`pricing_source`/`market_standing` from `contract.actionAuthority`, the GK-212 Hulk #273 axis/comp counts), `verified_comp_count` always NULL (disclosed, no such figure exists in the pricing system). 83/83 real tests across migration-contract/service-live-proof/handler-smoke/cross-device-proof, all self-cleaning against real Development; Production schema-only (0 rows, no writer exercised there). Full detail `docs/TICKET-REGISTRY.md`, GK-219/GK-220.
...
- D5B ValuationQuestion/Applicability design + 0015 scratch proof → `docs/adr/ADR-VALUATION-001-question-applicability.md`, `docs/D5B-0015-DESIGN-REPORT.md`
- HOLD outcome model (design only, not built) → `docs/HOLD-OUTCOME-MODEL-DESIGN.md`
