...
- **Test baseline:** 199 PASS / 19 FAIL / 4 TIMEOUT / 222 total — HEAD (this commit), 2026-08-20 (Directive AV, GK-133/GK-139). Prior 197/19/4/220 (AU fix-forward, `fdc240c`). Two new files, both passing: `grailkey-directive-av-title-megakey-authority.test.js` (28/28, unit-level reconcileTitle/isMegaKeyIdentityCorroborated coverage) and `grailkey-directive-av-handler-smoke.test.js` (28/28, GK-138 real-handler proof — Venom title adoption reaches PC/CV and never ID_REQUIRED, Dell'Otto mega-key floor STANDS DOWN instead of enforcing $300,000, a genuine corroborated 1963 ASM #1 still floors full force). FAIL list gained no new members and lost none — same 19 named files. Two real regressions were found and fixed mid-build (not shipped broken): the new title-adoption block initially fired on `family.titleAxisOnlyBlock===true` (the Q84-dual-axis-blocked shape, a qualified family deliberately trimmed of non-consensus content — different from the genuine no-candidate void this dispatch targets), corrected by excluding that flag from the gate; and a brittle source-text regex in `grailkey-directive-ag-22e-provenance-exemption.test.js` matched the old bare `shouldSkipAssemblyIntegrityCheck(...)` condition literally, updated to tolerate the new `|| identityTitleAdoptedContested` OR-clause while still verifying the skip branch's own body is unchanged. Three previously-undocumented pre-existing failures found this pass (confirmed failing identically with this dispatch's changes stashed out): `grailkey-directive-j-gk79a-relabel.test.js`, `grailkey-directive-p-task3-variant-on-card.test.js`, `grailkey-directive-q-variant-null-custody.test.js` — added to the named FAIL set below, not this dispatch's doing. `q-trackB-commit4.3-winning-family-authority.test.js` flagged once as FAIL in a sweep, passed cleanly (263/263) on two direct re-runs — a sweep-harness timeout-proximity artifact (13.2s runtime against a 20s cutoff), the same class already documented for `dispatch-42-comicvine-kill.test.js`/`grailkey-commit-m-pc-query-fallback.test.js`, not added as a new named failure. **Standing rule: re-stamp this line in the same commit as any test-file add/remove.** Full history: Pattern Library "GrailKey Directive AV."
